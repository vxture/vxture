...
function isFontSizePreference(
  value: unknown,
): value is ShellFontSizePreference {
...
function getRoleLabel(
  ...
  const fallback =
    user.roleNameEn?.trim() || user.roleLabel?.trim() || "Platform Architect";
...
function HeaderThemeToggle() {
  ...
  const currentTheme = isThemePreference(theme)
    ? theme
    : getGlobalUserPreferences().theme;
...
function QuickSettings() {
  ...
  const [selectedLocale, setSelectedLocale] = useState<Locale>(
    initialPrefs.locale || currentLocale || DEFAULT_LOCALE,
  );
  ...
  const [selectedDensity, setSelectedDensity] = useState<Density>(
    isDensity(initialPrefs.density)
      ? initialPrefs.density
      : isDensity(density)
        ? density
        : "default",
  );
  const [selectedFontSize, setSelectedFontSize] =
    useState<ShellFontSizePreference>(getStoredFontSizePreference);
...
function ShellFrame({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedSidebarCollapsed =
      window.localStorage.getItem(ADMIN_SIDEBAR_KEY) === "true";
  ...
  useEffect(() => {
    ...
    const updateAutoCollapse = () =>
      setSidebarAutoCollapsed(mediaQuery.matches);
    ...
    <div className={shellClassName} data-fullscreen-id={PAGE_FULLSCREEN_ID}>
      <header className="admin-shell__header">
        <div className="admin-shell-header__left">
              ...
              <div className="admin-workspace-switcher__list">
                {adminWorkspaces.map((workspace) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className="admin-workspace-switcher__icon"
                        aria-hidden="true"
                      >
                        <Icon
                          name={workspace.icon}
          ...
          <Link
            className="admin-shell-header__brand"
            href="/"
            aria-label={headerT("brandHome")}
          >
            <Image
          ...
          <strong className="admin-shell-header__workspace">
            {headerT("title")}
          </strong>
        ...
        <label
          ...
          aria-label={headerT("searchPlaceholder")}
        >
        ...
        <div className="admin-shell-header__actions">
          <div
            className="admin-shell-header__action-group"
            role="group"
            aria-label={headerT("quickPreferences")}
          >
            <HeaderThemeToggle />
            <HeaderLocaleSelect />
          ...
          <div
            className="admin-shell-header__action-group"
            role="group"
            aria-label={headerT("workspaceActions")}
          >
            <HeaderToolButton
              icon="help"
              title={headerT("help")}
            ...
            <HeaderToolButton
              icon="bell"
              title={headerT("notifications")}
              onClick={() => {}}
            />
            <HeaderToolButton
              icon="settings"
              title={headerT("settings")}
              onClick={() => {}}
            />
          </div>
      ...
      <div className="admin-shell__body">
        <aside
          ...
          aria-label={`${activeWorkspace.label}导航`}
        >
          <div className="admin-shell-sidebar__top">
            ...
            >
              <Icon
                name={
                  sidebarEffectiveCollapsed ? "text-indent" : "text-outdent"
                }
                size={24}
                fallback="placeholder"
              />
            ...
            {sidebarTextMounted ? (
              <strong className="admin-shell-sidebar__domain-title">
            ...
            {sidebarTextMounted ? (
              <Button
                ...
                aria-label={
                  allSectionsCollapsed ? "展开全部导航分组" : "收起全部导航分组"
                }
                title={
                  allSectionsCollapsed ? "展开全部导航分组" : "收起全部导航分组"
                }
          ...
          <nav className="admin-shell-nav">
            {activeSections.map((section) => {
              const sectionTitle = getTranslatedLabel(
                navigationT,
                `sections.${section.id}`,
                section.title,
              );
              ...
              return (
                <section
                  ...
                  className={[
                    ...
                    sectionCollapsed
                      ? "admin-shell-nav__section--collapsed"
                      : "",
                ...
                >
                  <SidebarTooltip
                    label={sectionTitle}
                    enabled={sidebarVisualCollapsed}
                  >
                    <Button
                      type="button"
                      variant="ghost"
                    ...
                    >
                      {sidebarTextMounted ? (
                        <span className="admin-shell-nav__title">
                          {sectionTitle}
                        </span>
                      ) : null}
                      <span
                        className="admin-shell-nav__section-icon admin-shell-nav__section-icon--expanded"
                        aria-hidden="true"
                      >
                        <Icon
                          name={
                            sectionCollapsed ? "chevron-right" : "chevron-down"
                          }
                          size="xs"
                      ...
                      <span
                        className="admin-shell-nav__section-icon admin-shell-nav__section-icon--collapsed"
                        aria-hidden="true"
                      >
                        <Icon
                          name={
                    ...
                    <div className="admin-shell-nav__items">
                      {section.items.map((item) => {
                        ...
                        const className = [
                          ...
                          item.disabled
                            ? "admin-shell-nav__item--disabled"
                            : "",
                        ]
                          .filter(Boolean)
                          .join(" ");
                          ...
                          <>
                            ...
                            {sidebarTextMounted ? (
                              <span>
                                ...
                                {item.status === "planned" ? (
                                  <em className="admin-shell-nav__status">
                                    待建设
                                  </em>
                                ) : null}
                              </span>
