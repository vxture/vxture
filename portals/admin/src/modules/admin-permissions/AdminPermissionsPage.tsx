...
} from "@/modules/shared/ViewModeSwitch";
...
function permissionMatchesFilters(
  ...
  if (
    ...
    permissionSource() !== filters.sourceFilter
  )
    return false;
  if (
    normalizedQuery &&
...
function PermissionDetailDialog({
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`${permissionDisplayName(permission)} 权限详情`}
    >
      <div
        className="vx-admin-role-permission-dialog__backdrop"
        onClick={onClose}
      />
