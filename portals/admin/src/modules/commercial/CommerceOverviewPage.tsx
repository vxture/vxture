...
import type { CommerceOverviewMetric, CommerceOverviewSnapshot } from '@/entities/console';
...
function metricValue(metric: CommerceOverviewMetric) {
  if (typeof metric.amount === 'number') return formatCurrency(metric.amount, metric.currency ?? 'CNY');
  return formatNumber(metric.value);
}

function metricTags(metric: CommerceOverviewMetric) {
  if (typeof metric.amount === 'number') return [`${formatNumber(metric.value)} 笔`, metric.hint];
  return [metric.hint];
}

function riskIcon(tone: CommerceOverviewSnapshot['risks'][number]['tone']): IconName {
...
function RiskPanel({ snapshot }: { snapshot: CommerceOverviewSnapshot }) {
  return (
    <section className="vx-commerce-panel vx-commerce-risk-panel" aria-label="商业风险">
      ...
      <div className="vx-commerce-risk-list">
        {snapshot.risks.map((risk) => (
          <Link key={risk.id} className={`vx-commerce-risk-item vx-commerce-risk-item--${risk.tone}`} href={risk.href}>
            <span className="vx-commerce-risk-item__icon" aria-hidden="true">
              <Icon name={riskIcon(risk.tone)} size="sm" fallback="placeholder" />
            </span>
            <span>
...
function PlanRevenuePanel({ snapshot }: { snapshot: CommerceOverviewSnapshot }) {
  return (
    <section className="vx-commerce-panel vx-commerce-plan-panel" aria-label="套餐收入">
      <header className="vx-commerce-panel__header">
        <div className="admin-overview-heading">
          <span className="admin-overview-heading__icon" aria-hidden="true">
          ...
          <div className="admin-overview-heading__copy">
            <h2>套餐收入</h2>
            <p>按服务套餐汇总订阅数量、应收、已收和优惠减免。</p>
          </div>
        </div>
        <Link href="/service-plans">套餐管理</Link>
      </header>
      <div className="vx-commerce-plan-list">
        {snapshot.planRevenue.length ? snapshot.planRevenue.map((plan) => (
          <article key={`${plan.planName}-${plan.tierName}`} className="vx-commerce-plan-row">
            <span className="vx-commerce-plan-row__main">
              <strong>{plan.planName}</strong>
              <span>
                <Tag tone={tierTone(plan.tierName)}>{plan.tierName}</Tag>
                <small>{formatNumber(plan.subscriptionCount)} 个订阅</small>
              </span>
            </span>
            <span className="vx-commerce-plan-row__amounts">
              <span><strong>{formatCurrency(plan.revenueAmount, plan.currency)}</strong><small>应收</small></span>
              <span><strong>{formatCurrency(plan.paidAmount, plan.currency)}</strong><small>已收</small></span>
              <span><strong>{formatCurrency(plan.discountAmount, plan.currency)}</strong><small>减免</small></span>
            </span>
          </article>
        )) : (
...
function QuickLinkPanel() {
  return (
    <section className="vx-commerce-panel vx-commerce-link-panel" aria-label="商业财务入口">
      <header className="vx-commerce-panel__header">
        <div className="admin-overview-heading">
          <span className="admin-overview-heading__icon" aria-hidden="true">
      ...
      <div className="vx-commerce-link-grid">
        {quickLinks.map((link) => (
          <Link key={link.href} className="vx-commerce-link-card" href={link.href}>
...
export function CommerceOverviewPage() {
  const [snapshot, setSnapshot] = useState<CommerceOverviewSnapshot | null>(null);
  ...
  return (
    <div className="vx-page-stack vx-tenant-management-page vx-commerce-overview-page">
      ...
      {loading && !snapshot ? <div className="vx-commerce-panel__empty">正在读取商业财务快照</div> : null}
