...
function billingExceptionTags(bill: BillingRecord) {
  ...
  if (bill.billType === 'adjust') {
    tags.push({ key: 'adjust', label: '调整单', tone: 'adjust', title: bill.operationRemark ?? undefined });
  }
  if (bill.billType === 'supplement') {
    tags.push({ key: 'supplement', label: '补录单', tone: 'supplement', title: bill.operationRemark ?? undefined });
  }
  if (bill.discountAmount > 0) {
    tags.push({ key: 'discounted', label: '已减免', tone: 'discount', title: `减免 ${formatCurrency(bill.discountAmount, bill.currency)}` });
  }
  ...
  if (bill.billStatus === 'cancelled') {
    tags.push({ key: 'cancelled', label: '已作废', tone: 'cancelled', title: bill.operationRemark ?? undefined });
  }
  if (bill.invoiceStatus === 'red' || bill.invoiceStatus === 'rejected') {
    tags.push({ key: 'invoice_exception', label: bill.invoiceStatus === 'red' ? '发票红冲' : '发票驳回', tone: 'invoice', title: bill.invoiceNo ?? undefined });
  }
...
function matchesBillingExceptionFilter(bill: BillingRecord, filter: BillingExceptionFilter) {
...
function tierFilterValue(record: BillingRecord): TierFilter {
  ...
  if (tierName === 'enterprise' || record.tierName === 'Enterprise') return 'enterprise';
...
function BillingActionsMenu({
  ...
  return (
    <div className="vx-tenant-actions" onClick={(event) => event.stopPropagation()} onMouseLeave={onClose}>
      <button className="vx-tenant-actions__trigger" type="button" aria-label={`${bill.billNo} 账单操作`} title="操作" onClick={onToggle}>
...
function BillingListRows({
  ...
  return (
    <div className="vx-tenant-directory-list vx-billing-directory-list" role="region" aria-label="账单发票清单">
      ...
      {bills.map((bill, index) => (
        <div
          key={bill.id}
          className={joinClasses('vx-tenant-directory-row', `vx-billing-row--${bill.billStatus}`)}
          role="button"
          tabIndex={0}
          onClick={() => router.push(`/billing/${encodeURIComponent(bill.id)}`)}
          onKeyDown={(event) => {
            if (event.key === 'Enter') router.push(`/billing/${encodeURIComponent(bill.id)}`);
          }}
        >
          ...
          <span className="vx-billing-row__bill">
            ...
            <small>{cycleLabel(bill.billCycle)} · {formatDate(bill.cycleStartDate)} - {formatDate(bill.cycleEndDate)}</small>
          </span>
          <span className="vx-billing-row__tenant">
            <Icon name={bill.tenantType === 'company' ? 'buildings' : 'user'} size="sm" fallback="placeholder" />
            <span>
              <strong>{bill.tenantName}</strong>
              <small>{bill.tenantCode} · {typeLabel(bill.tenantType)}</small>
            </span>
          </span>
          <span className="vx-billing-row__plan">
            <span className="vx-tenant-directory-row__tag-line">
              <Badge className={`vx-tenant-pill vx-billing-pill--tier-${tierFilterValue(bill)}`}>{bill.tierName ?? '未关联'}</Badge>
            </span>
            <small>{bill.servicePlanName ?? bill.orderNo ?? '未关联订阅'}</small>
          </span>
          <span className="vx-billing-row__amount">
            <strong>{formatCurrency(bill.payableAmount, bill.currency)}</strong>
            <small>{bill.discountAmount > 0 ? `原价 ${formatCurrency(bill.totalAmount, bill.currency)} · 减免 ${formatCurrency(bill.discountAmount, bill.currency)}` : `原价 ${formatCurrency(bill.totalAmount, bill.currency)}`}</small>
          </span>
          <span className="vx-billing-row__exception">
            {billingExceptionTags(bill).length ? (
              <span className="vx-billing-exception-tags">
                {billingExceptionTags(bill).map((tag) => (
                  <Badge key={tag.key} className={`vx-tenant-pill vx-billing-exception-pill vx-billing-exception-pill--${tag.tone}`} title={tag.title}>
                    {tag.label}
                  </Badge>
                ))}
              </span>
            ...
            {bill.operationRemark ? <small title={bill.operationRemark}>{bill.operationRemark}</small> : null}
          </span>
          <span className="vx-billing-row__payment">
            <span className="vx-billing-status-line">
              <span className={`vx-billing-status-dot vx-billing-status-dot--${bill.billStatus}`} role="img" aria-label={billStatusLabel(bill.billStatus)}>
                <Icon name={billStatusIcon(bill.billStatus)} size="xs" fallback="placeholder" />
              </span>
              <Badge className={`vx-tenant-pill vx-billing-pill--${bill.billStatus}`}>{billStatusLabel(bill.billStatus)}</Badge>
            </span>
            <small>已收 {formatCurrency(bill.paidAmount, bill.currency)}</small>
          </span>
          <span className="vx-billing-row__invoice">
            <Badge className={`vx-tenant-pill vx-billing-pill--invoice-${bill.invoiceStatus}`}>{invoiceStatusLabel(bill.invoiceStatus)}</Badge>
            <small>{bill.invoiceNo ?? `已登记 ${formatCurrency(bill.invoicedAmount, bill.currency)}`}</small>
          </span>
          <BillingActionsMenu
            bill={bill}
...
function BillingCards({
  ...
  return (
    <div className="vx-tenant-directory-cards vx-billing-cards" aria-label="账单发票卡片">
      {bills.map((bill) => (
        <article
          key={bill.id}
          className={joinClasses('vx-tenant-directory-card', `vx-billing-card--${bill.billStatus}`)}
          role="button"
          tabIndex={0}
          onClick={() => router.push(`/billing/${encodeURIComponent(bill.id)}`)}
          onKeyDown={(event) => {
        ...
        >
          <header>
            ...
            <div>
              <strong>{bill.billNo}</strong>
              <span>{bill.tenantName} · {bill.tierName ?? '未关联套餐'}</span>
            </div>
            <BillingActionsMenu
              bill={bill}
              open={openMenuId === bill.id}
          ...
          <p className="vx-billing-card__plan">{bill.servicePlanName ?? bill.orderNo ?? '未关联订阅'}</p>
          ...
          <footer>
            <span>{formatDate(bill.cycleStartDate)} - {formatDate(bill.cycleEndDate)}</span>
...
function Pagination({
  ...
  return (
    <footer className="vx-tenant-pagination">
      <span className="vx-tenant-pagination__total">共 {formatNumber(total)} 条账单记录</span>
...
export function BillingPage() {
  ...
  const [exceptionFilter, setExceptionFilter] = useState<BillingExceptionFilter>('all');
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<PageSize>(20);
  const [loading, setLoading] = useState(true);
  const [invoiceTarget, setInvoiceTarget] = useState<BillingRecord | null>(null);
  ...
  const filteredBills = useMemo(() => {
    ...
    return bills.filter((bill) => {
      if (billStatusFilter !== 'all' && bill.billStatus !== billStatusFilter) return false;
      if (invoiceStatusFilter !== 'all' && bill.invoiceStatus !== invoiceStatusFilter) return false;
      if (billTypeFilter !== 'all' && bill.billType !== billTypeFilter) return false;
      if (tierFilter !== 'all' && tierFilterValue(bill) !== tierFilter) return false;
      if (!matchesBillingExceptionFilter(bill, exceptionFilter)) return false;
      if (normalizedQuery && !billingSearchText(bill).includes(normalizedQuery)) return false;
      return true;
    });
  }, [billStatusFilter, billTypeFilter, bills, exceptionFilter, invoiceStatusFilter, query, tierFilter]);

  const pageCount = Math.max(1, Math.ceil(filteredBills.length / pageSize));
  const activePage = Math.min(currentPage, pageCount);
  const visibleBills = filteredBills.slice((activePage - 1) * pageSize, activePage * pageSize);
  const receivableAmount = bills.reduce((sum, item) => sum + item.payableAmount, 0);
  const paidAmount = bills.reduce((sum, item) => sum + item.paidAmount, 0);
  const pendingCount = bills.filter((item) => item.billStatus === 'unpaid' || item.billStatus === 'paying' || item.billStatus === 'partial' || item.billStatus === 'overdue').length;
  const invoicePendingCount = bills.filter((item) => item.invoiceStatus === 'none' || item.invoiceStatus === 'applying' || item.invoiceStatus === 'auditing').length;
  const invoicedAmount = bills.reduce((sum, item) => sum + item.invoicedAmount, 0);
  const exceptionCount = bills.filter(hasBillingException).length;
  const discountedAmount = bills.reduce((sum, item) => sum + item.discountAmount, 0);
  const exceptionBillCount = bills.filter((item) => item.billType === 'adjust' || item.billType === 'supplement').length;

  useEffect(() => {
    setCurrentPage(1);
    setOpenMenuId(null);
  }, [billStatusFilter, billTypeFilter, exceptionFilter, invoiceStatusFilter, pageSize, query, tierFilter, viewMode]);
  ...
  async function handleSyncOfflineInvoice(payload: Parameters<typeof syncOfflineInvoice>[1]) {
    ...
    try {
      ...
      setOperationError(error instanceof Error ? error.message : '线下发票登记失败，请稍后重试。');
    } finally {
  ...
  return (
    <div className="vx-page-stack vx-tenant-management-page vx-billing-page">
      ...
      <section className="vx-tenant-summary" aria-label="账单发票统计">
        <SummaryItem icon="key" label="账单总数" value={formatNumber(bills.length)} tags={[`筛选 ${formatNumber(filteredBills.length)}`, `异常 ${formatNumber(exceptionCount)}`]} />
        <SummaryItem icon="clock" label="待收款" value={formatNumber(pendingCount)} tags={[`逾期 ${formatNumber(bills.filter((item) => item.billStatus === 'overdue').length)}`]} tone={pendingCount ? 'amber' : 'green'} />
        <SummaryItem icon="chart-bar" label="应收金额" value={formatCurrency(receivableAmount, 'CNY')} tags={[`已收 ${formatCurrency(paidAmount, 'CNY')}`, `减免 ${formatCurrency(discountedAmount, 'CNY')}`]} tone="green" />
        <SummaryItem icon="table" label="开票进度" value={formatCurrency(invoicedAmount, 'CNY')} tags={[`待处理 ${formatNumber(invoicePendingCount)}`, `调整 ${formatNumber(exceptionBillCount)}`]} tone={invoicePendingCount ? 'amber' : 'green'} />
      </section>
      ...
      <div className="vx-tenant-list-shell">
        <section className="vx-tenant-toolbar" aria-label="账单筛选">
          <ViewModeSwitch value={viewMode} onChange={setViewMode} ariaLabel="账单展示方式" />
          <span className="vx-tenant-view-count">{formatNumber(filteredBills.length)}</span>
          ...
          <div className="vx-tenant-filters">
            <select className="vx-input vx-tenant-select" value={billStatusFilter} onChange={(event) => setBillStatusFilter(event.target.value as BillStatusFilter)} aria-label="账单状态">
            ...
            <select className="vx-input vx-tenant-select" value={invoiceStatusFilter} onChange={(event) => setInvoiceStatusFilter(event.target.value as InvoiceStatusFilter)} aria-label="发票状态">
            ...
            <select className="vx-input vx-tenant-select" value={billTypeFilter} onChange={(event) => setBillTypeFilter(event.target.value as BillTypeFilter)} aria-label="账单类型">
            ...
            <select className="vx-input vx-tenant-select" value={exceptionFilter} onChange={(event) => setExceptionFilter(event.target.value as BillingExceptionFilter)} aria-label="处理类型">
          ...
          {visibleBills.length ? (
            ...
            <section className="vx-tenant-empty">
              <EmptyState
                ...
                description={loading ? '正在读取账单、收款和发票登记数据。' : '清空筛选条件后可查看全部账单记录。'}
                action={
