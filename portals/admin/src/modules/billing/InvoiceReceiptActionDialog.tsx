...
export function canRunInvoiceReceiptAction(action: BillingInvoiceReceiptAction, receipt: BillingInvoiceReceiptRecord) {
  if (receipt.invoiceStatus === 'red' || receipt.invoiceStatus === 'rejected') return false;
  if (action === 'red') return true;
  if (action === 'finish') return receipt.invoiceStatus === 'issued' || receipt.invoiceStatus === 'sending';
  return receipt.invoiceStatus === 'issued' || receipt.invoiceStatus === 'sending' || receipt.invoiceStatus === 'finished';
}

export function invoiceReceiptActionDisabledReason(action: BillingInvoiceReceiptAction, receipt: BillingInvoiceReceiptRecord) {
  if (receipt.invoiceStatus === 'red') return '已红冲发票不能继续操作。';
  if (receipt.invoiceStatus === 'rejected') return '已驳回发票不能继续操作。';
  if (action === 'finish' && receipt.invoiceStatus === 'finished') return '发票已完成。';
  if (!canRunInvoiceReceiptAction(action, receipt)) return '当前发票状态不支持该操作。';
  return null;
}
...
function actionDescription(action: BillingInvoiceReceiptAction) {
  if (action === 'update_shipping') return '记录线下发票的快递公司、快递单号和寄送时间，不调用任何在线开票或物流接口。';
  if (action === 'finish') return '确认线下发票已完成交付或归档，可选补充快递信息。';
...
export function InvoiceReceiptActionDialog({
  ...
  const [expressCompany, setExpressCompany] = useState(receipt.expressCompany ?? '');
  const [expressNo, setExpressNo] = useState(receipt.expressNo ?? '');
  const [sendAt, setSendAt] = useState(localDateTimeValue(receipt.sendAt ? new Date(receipt.sendAt) : new Date()));
  ...
  const canSubmit =
    ...
    (!requiresShipping || (expressCompany.trim().length > 0 && expressNo.trim().length > 0));

  useEffect(() => {
    setExpressCompany(receipt.expressCompany ?? '');
    setExpressNo(receipt.expressNo ?? '');
    setSendAt(localDateTimeValue(receipt.sendAt ? new Date(receipt.sendAt) : new Date()));
    setStatusRemark('');
  }, [action, receipt]);

  return (
    <div className="vx-subscription-action-dialog" role="dialog" aria-modal="true" aria-labelledby="invoice-receipt-action-title">
      <form
      ...
      >
        <header>
          <span aria-hidden="true" className={`vx-subscription-action-dialog__icon ${actionIconClass(action)}`}>
            <Icon name={action === 'red' ? 'warning' : action === 'finish' ? 'check' : 'key'} size="lg" fallback="placeholder" />
          </span>
          <div>
            <h2 id="invoice-receipt-action-title">{invoiceReceiptActionLabel(action)}</h2>
            <p>{receipt.invoiceNo} · {receipt.invoiceTitle}</p>
          </div>
        </header>
