...
function invoiceStatusLabel(status: Extract<BillingInvoiceStatus, 'issued' | 'sending' | 'finished'>) {
...
export function OfflineInvoiceDialog({
  ...
  const [invoiceType, setInvoiceType] = useState<BillingInvoiceType>('normal_vat');
  const [invoiceTaxType, setInvoiceTaxType] = useState<BillingInvoiceTaxType>('enterprise');
  const [invoiceTitle, setInvoiceTitle] = useState(bill.tenantName);
  const [taxNo, setTaxNo] = useState('');
  const [invoiceAmount, setInvoiceAmount] = useState(String(remainingAmount || bill.payableAmount));
  const [taxAmount, setTaxAmount] = useState('0');
  const [invoiceStatus, setInvoiceStatus] = useState<Extract<BillingInvoiceStatus, 'issued' | 'sending' | 'finished'>>('issued');
  ...
  useEffect(() => {
    ...
    setTaxNo('');
    setInvoiceAmount(String(remainingInvoiceAmount(bill) || bill.payableAmount));
    setTaxAmount('0');
  ...
  return (
    <div className="vx-subscription-action-dialog" role="dialog" aria-modal="true" aria-labelledby="offline-invoice-dialog-title">
      <form
      ...
      >
        <header>
          <span aria-hidden="true" className="vx-subscription-action-dialog__icon vx-subscription-action-dialog__icon--renew">
            <Icon name="key" size="lg" fallback="placeholder" />
          </span>
          <div>
            <h2 id="offline-invoice-dialog-title">登记线下发票</h2>
            <p>{bill.billNo} · 剩余可开票 {formatCurrency(remainingAmount, bill.currency)}</p>
          </div>
        </header>
