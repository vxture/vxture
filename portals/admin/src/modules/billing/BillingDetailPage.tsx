  ...
  return (
    <section className="vx-product-capability-summary">
      <div className="vx-product-capability-summary__identity">
        ...
        <div>
          <h2>{bill.billNo}</h2>
          <p>{bill.tenantName} / {bill.tierName ?? '未关联套餐'}</p>
      ...
      <div className="vx-product-capability-summary__metrics">
        <DetailMetric label="账单应收" value={formatCurrency(bill.payableAmount, bill.currency)} tag={billTypeLabel(bill.billType)} />
        <DetailMetric label="已收金额" value={formatCurrency(bill.paidAmount, bill.currency)} tag={bill.paymentMethod ?? '未收款'} />
        <DetailMetric label="已开票" value={formatCurrency(bill.invoicedAmount, bill.currency)} tag={bill.invoiceNo ?? invoiceStatusLabel(bill.invoiceStatus)} />
        <DetailMetric label="账期" value={`${formatDate(bill.cycleStartDate)} - ${formatDate(bill.cycleEndDate)}`} tag={cycleLabel(bill.billCycle)} />
      </div>
    </section>
  );
}
...
function BillingDetails({
  ...
  onReceiptAction: (receipt: BillingInvoiceReceiptRecord, action: BillingInvoiceReceiptAction) => void;
}) {
  return (
    <section className="vx-product-capability-detail" aria-label={`${bill.billNo} 账单详情`}>
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          ...
          <DetailField label="账单状态" value={billStatusLabel(bill.billStatus)} />
          <DetailField label="账单类型" value={billTypeLabel(bill.billType)} />
          <DetailField label="账期类型" value={cycleLabel(bill.billCycle)} />
          <DetailField label="账期开始" value={formatDate(bill.cycleStartDate)} />
          ...
          <DetailField label="运营备注" value={bill.operationRemark ?? '未设置'} />
        </div>
      </section>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          ...
          <DetailField label="服务套餐" value={bill.servicePlanName ?? '未关联'} />
          <DetailField label="套餐版本" value={bill.tierName ?? '未关联'} />
          <DetailField label="订单编号" value={bill.orderNo ?? '未关联'} />
          <DetailField label="订阅 ID" value={bill.subscriptionId ?? '未关联'} />
        </div>
        <div className="vx-product-capability-actions vx-subscription-detail-links">
          <Link href={`/tenants/${encodeURIComponent(bill.tenantId)}`} className="vx-btn vx-btn--outline vx-btn--default">
            <Icon name="buildings" size="xs" fallback="placeholder" />
            租户详情
          </Link>
          {bill.subscriptionId ? (
            <>
              <Link href={`/subscriptions/${encodeURIComponent(bill.subscriptionId)}`} className="vx-btn vx-btn--outline vx-btn--default">
                <Icon name="star" size="xs" fallback="placeholder" />
                订阅详情
              </Link>
              <Link href={`/orders/${encodeURIComponent(bill.subscriptionId)}`} className="vx-btn vx-btn--outline vx-btn--default">
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          <DetailField label="账单原价" value={formatCurrency(bill.totalAmount, bill.currency)} />
          <DetailField label="优惠金额" value={formatCurrency(bill.discountAmount, bill.currency)} />
          <DetailField label="应收金额" value={formatCurrency(bill.payableAmount, bill.currency)} />
          <DetailField label="已收金额" value={formatCurrency(bill.paidAmount, bill.currency)} />
          <DetailField label="剩余应收" value={formatCurrency(Math.max(0, bill.payableAmount - bill.paidAmount), bill.currency)} />
          <DetailField label="收款时间" value={formatDate(bill.paidAt)} />
          <DetailField label="支付方式" value={bill.paymentMethod ?? '未设置'} />
          <DetailField label="交易流水" value={bill.transactionNo ?? '未设置'} />
        ...
        <div className="vx-product-detail-list vx-product-detail-list--entitlements">
          {bill.invoiceItems.map((item) => (
            <div key={item.id} className="vx-product-detail-list__row">
              ...
              <p>{item.remark ?? `单价 ${formatCurrency(item.unitPrice, bill.currency)}`}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-detail-list vx-product-detail-list--entitlements">
          {bill.paymentRecords.length ? bill.paymentRecords.map((payment) => (
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-detail-list vx-product-detail-list--entitlements">
          {bill.invoiceReceipts.length ? bill.invoiceReceipts.map((receipt) => (
            <div key={receipt.id} className="vx-product-detail-list__row">
              ...
              <small>{invoiceTypeLabel(receipt.invoiceType)} | {taxTypeLabel(receipt.invoiceTaxType)} | {invoiceStatusLabel(receipt.invoiceStatus)}</small>
              <em>{formatCurrency(receipt.invoiceAmount, receipt.currency)}</em>
              <p className="vx-billing-receipt-actions">
                <span>
                  {receipt.invoiceFileUrl ? (
                    <a href={receipt.invoiceFileUrl} target="_blank" rel="noreferrer">查看发票文件</a>
                  ) : receipt.expressNo ? (
            ...
            <div className="vx-product-detail-list__row">
              ...
              <em>{formatCurrency(Math.max(0, bill.payableAmount - bill.invoicedAmount), bill.currency)}</em>
...
export function BillingDetailPage({ billId }: { billId: string }) {
  ...
  const [receiptActionTarget, setReceiptActionTarget] = useState<{ receipt: BillingInvoiceReceiptRecord; action: BillingInvoiceReceiptAction } | null>(null);
  const [submittingReceiptAction, setSubmittingReceiptAction] = useState(false);
  const [billActionTarget, setBillActionTarget] = useState<BillingBillAction | null>(null);
  ...
  function requestReceiptAction(receipt: BillingInvoiceReceiptRecord, action: BillingInvoiceReceiptAction) {
    setOperationError(null);
  ...
  async function handleSubmitReceiptAction(payload: Parameters<typeof submitBillingInvoiceReceiptAction>[2]) {
    ...
    try {
      const updatedBill = await submitBillingInvoiceReceiptAction(bill.id, receiptActionTarget.receipt.id, payload);
      setBill(updatedBill);
  ...
  async function handleSubmitBillAction(payload: Parameters<typeof submitBillingBillAction>[1]) {
    ...
    try {
      const updatedBill = await submitBillingBillAction(bill.id, payload);
      const actionLabel = billingBillActionLabel(billActionTarget);
      setBill(updatedBill);
      setOperationFeedback(updatedBill.id === bill.id ? `${actionLabel}已完成。` : `${actionLabel}已完成，已跳转到新账单。`);
      ...
      setOperationError(error instanceof Error ? error.message : '账单异常处理失败，请稍后重试。');
    } finally {
  ...
  if (!loading && !bill) {
    return (
      <div className="vx-page-stack vx-product-capability-page">
        ...
        />
        <EmptyState title="账单不存在" description="该账单可能已归档，或当前账号无权访问。" />
      </div>
    );
  }
  ...
  return (
    <div className="vx-page-stack vx-product-capability-page vx-billing-detail-page">
      <PageHeader
        ...
        description={bill ? `${bill.tenantName} · ${bill.servicePlanName ?? '未关联套餐'} · ${invoiceStatusLabel(bill.invoiceStatus)}` : '正在读取账单、收款和发票登记数据。'}
        action={
          <div className="vx-product-capability-actions">
            ...
            {bill ? (
              <>
                {(['mark_overdue', 'discount', 'create_adjustment', 'create_supplement', 'cancel'] as const).map((action) => (
                  <Button
                    key={action}
                    variant="outline"
                    className={action === 'cancel' ? 'vx-subscription-action-button--danger' : undefined}
                    onClick={() => requestBillAction(action)}
                    disabled={!canRunBillingBillAction(action, bill)}
                    title={billingBillActionDisabledReason(action, bill) ?? undefined}
                  >
                    <Icon name={action === 'cancel' ? 'warning' : action === 'mark_overdue' ? 'clock' : action === 'discount' ? 'chart-bar' : action === 'create_adjustment' ? 'edit' : 'plus'} size="xs" fallback="placeholder" />
