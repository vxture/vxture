...
export function canRunBillingBillAction(action: BillingBillAction, bill: BillingDetailRecord) {
  if (action === 'create_adjustment' || action === 'create_supplement') return true;
  if (bill.billStatus === 'cancelled') return false;
  if (action === 'cancel') return bill.paidAmount <= 0 && bill.invoicedAmount <= 0;
...
export function billingBillActionDisabledReason(action: BillingBillAction, bill: BillingDetailRecord) {
  if (action === 'create_adjustment' || action === 'create_supplement') return null;
  if (bill.billStatus === 'cancelled') return '已作废账单不能继续处理。';
  if (action === 'cancel' && bill.paidAmount > 0) return '已有收款的账单不能直接作废。';
  if (action === 'cancel' && bill.invoicedAmount > 0) return '已有有效发票的账单需先完成红冲/作废登记。';
  if (action === 'mark_overdue' && bill.billStatus === 'paid') return '已结清账单不能标记逾期。';
  if (action === 'discount' && bill.totalAmount <= 0) return '零金额账单不需要应收减免。';
  if (action === 'discount' && bill.payableAmount <= bill.paidAmount) return '当前应收已不高于已收金额。';
  if (action === 'discount' && bill.payableAmount <= bill.invoicedAmount) return '当前应收已不高于已开票金额。';
  if (!canRunBillingBillAction(action, bill)) return '当前账单状态不支持该操作。';
  return null;
}
...
function actionDescription(action: BillingBillAction) {
  if (action === 'cancel') return '用于确认为误生成、重复生成或无需追收的账单。已有收款或有效发票时不能直接作废。';
  if (action === 'discount') return '用于登记线下审批通过的应收减免，只调整账单应收，不处理退款或发票红冲。';
  if (action === 'mark_overdue') return '用于登记逾期跟进原因，帮助后续催收、展期或客户成功介入。';
  if (action === 'create_adjustment') return '用于针对当前账单新建一张调整单，承载差额修正、服务变更等运营调整。';
...
function actionIconClass(action: BillingBillAction) {
  ...
  if (action === 'discount' || action === 'mark_overdue') return 'vx-subscription-action-dialog__icon--renew';
  if (action === 'create_supplement') return 'vx-subscription-action-dialog__icon--resume';
  return '';
}
...
export function BillingBillActionDialog({
  ...
  const [cycleStartDate, setCycleStartDate] = useState(localDateValue(bill.cycleStartDate));
  const [cycleEndDate, setCycleEndDate] = useState(localDateValue(bill.cycleEndDate));
  const isDiscount = action === 'discount';
  const isExceptionBill = action === 'create_adjustment' || action === 'create_supplement';
  const normalizedDiscountAmount = Number(discountAmount);
  const normalizedAmount = Number(amount);
  const maxDiscountAmount = Math.max(0, Math.min(
    bill.payableAmount,
    bill.payableAmount - bill.paidAmount,
    bill.payableAmount - bill.invoicedAmount,
  ));
  const canSubmit = useMemo(() => {
    ...
    if (isDiscount) return Number.isFinite(normalizedDiscountAmount) && normalizedDiscountAmount > 0 && normalizedDiscountAmount <= maxDiscountAmount + 0.01;
    if (isExceptionBill) {
      return (
        itemName.trim().length >= 2 &&
        Number.isFinite(normalizedAmount) &&
        normalizedAmount > 0 &&
        (!cycleStartDate || !cycleEndDate || new Date(cycleEndDate).getTime() >= new Date(cycleStartDate).getTime())
      );
    }
    return true;
  }, [busy, cycleEndDate, cycleStartDate, isDiscount, isExceptionBill, itemName, maxDiscountAmount, normalizedAmount, normalizedDiscountAmount, reason]);
  ...
  return (
    <div className="vx-subscription-action-dialog" role="dialog" aria-modal="true" aria-labelledby="billing-bill-action-title">
      <form
        ...
        onSubmit={(event) => {
          ...
          onSubmit({
            action,
            reason: reason.trim(),
            discountAmount: isDiscount ? Math.round(normalizedDiscountAmount * 100) / 100 : null,
            amount: isExceptionBill ? Math.round(normalizedAmount * 100) / 100 : null,
      ...
      >
        <header>
          <span aria-hidden="true" className={`vx-subscription-action-dialog__icon ${actionIconClass(action)}`}>
            <Icon name={actionIconName(action)} size="lg" fallback="placeholder" />
          </span>
          <div>
            <h2 id="billing-bill-action-title">{billingBillActionLabel(action)}</h2>
            <p>{bill.billNo} · 当前应收 {formatCurrency(bill.payableAmount, bill.currency)}</p>
          </div>
        </header>
        <p className="vx-subscription-action-dialog__description">{actionDescription(action)}</p>
