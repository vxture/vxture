...
function TodoRow({
  ...
  return (
    <div
      className={`vx-tenant-directory-row vx-tenant-directory-row--${item.severity} vx-ops-todo-row vx-ops-todo-operation-row ${selected ? "vx-ops-todo-operation-row--selected" : ""}`}
      onClick={(event) => {
...
export function OpsTodosPage() {
  ...
  const selectedVisibleTodoCount = todos.filter((todo) =>
  ...
  function toggleTodoPageSelection(checked: boolean) {
    setSelectedTodoIds((current) => {
      const next = new Set(current);
      todos.forEach((todo) => {
