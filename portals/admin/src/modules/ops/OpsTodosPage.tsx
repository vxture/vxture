...
import { formatNumber, riskLabel, statusLabel, typeLabel, verifiedLabel } from '@/modules/tenants/tenant-utils';
...
function buildOpsTodos(): OpsTodoItem[] {
  return tenantOperationRecords
    .flatMap((tenant) => {
      ...
      if (tenant.riskLevel !== 'normal' || tenant.status === 'suspended') {
        items.push({
          ...
          tenantMeta,
          href: tenantHref,
          severity: tenant.riskLevel === 'high' || tenant.status === 'suspended' ? 'rose' : 'amber',
      ...
      tenant.tickets
        ...
        .forEach((ticket) => {
          items.push({
            ...
            tenantMeta,
            href: tenantHref,
            severity: ticket.priority === 'p0' || ticket.status === 'blocked' ? 'rose' : ticket.priority === 'p1' ? 'amber' : 'blue',
            priority: ticket.priority === 'p0' ? 1 : ticket.priority === 'p1' ? 10 : ticket.priority === 'p2' ? 30 : 50,
      ...
      tenant.usage
        ...
        .forEach((usage) => {
          const usageRate = usage.quota ? Math.round((usage.used / usage.quota) * 100) : 0;
          items.push({
      ...
      tenant.subscriptions
        .filter((subscription) => subscription.status === 'past_due' || subscription.status === 'trial')
    ...
    .sort((left, right) => {
      ...
      return left.priority - right.priority || new Date(right.updatedAt).getTime() - new Date(left.updatedAt).getTime();
    });
}
...
function TodoTypeSummary({ type, count }: { type: TodoType; count: number }) {
  return (
    <Link className="vx-tenant-list-card vx-ops-todo-type-card" href={TODO_TYPE_HREF[type]}>
...
function TodoRow({ item, index }: { item: OpsTodoItem; index: number }) {
  return (
    <Link className={`vx-tenant-directory-row vx-tenant-directory-row--${item.severity} vx-ops-todo-row`} href={item.href}>
      ...
      <span className="vx-tenant-directory-row__tag-line">
        {item.tags.slice(0, 3).map((tag) => (
          <Badge key={tag} className={`vx-tenant-pill vx-tenant-pill--${item.severity === 'rose' ? 'danger' : item.severity === 'amber' ? 'warning' : 'normal'}`}>
            {tag}
          </Badge>
        ))}
      </span>
  ...
  return (
    <div className="vx-page-stack vx-tenant-management-page vx-ops-todos-page">
      ...
      <section className="vx-tenant-summary" aria-label="运营待办统计">
        <SummaryItem icon="warning" label="紧急事项" value={formatNumber(urgentTodos.length)} tags={[`影响租户 ${formatNumber(affectedTenants)}`]} tone={urgentTodos.length ? 'rose' : 'green'} />
        <SummaryItem icon="medal" label="认证待审" value={formatNumber(verificationTodos.length)} tags={['组织资质']} tone={verificationTodos.length ? 'amber' : 'green'} />
        <SummaryItem icon="chat-circle" label="未关闭工单" value={formatNumber(ticketTodos.length)} tags={[`P0/P1 ${formatNumber(ticketTodos.filter((todo) => todo.priority <= 10).length)}`]} tone={ticketTodos.length ? 'amber' : 'green'} />
        <SummaryItem icon="table" label="全部待办" value={formatNumber(todos.length)} tags={['按优先级排序']} />
      </section>

      <section className="vx-tenant-workspace vx-ops-todos-workspace" aria-label="运营待办工作台">
        <aside className="vx-tenant-list vx-ops-todo-type-list" aria-label="待办分类">
          <header>
            <strong>待办分类</strong>
            <span>按业务入口分流</span>
          </header>
          <div>
            {typeCounts.map((item) => (
              <TodoTypeSummary key={item.type} type={item.type} count={item.count} />
            ))}
          </div>
        </aside>
