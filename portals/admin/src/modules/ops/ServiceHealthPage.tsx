...
function serviceProbe(service: DevServiceSnapshot): ProbeFilter {
  const hasTcp = service.health.some((check) => check.url.startsWith('tcp://') || check.okStatuses === null);
  const hasHttp = service.health.some((check) => check.url.startsWith('http://') || check.url.startsWith('https://'));
...
function serviceSearchText(service: DevServiceSnapshot) {
  return [
    ...
    ...service.health.flatMap((check) => [check.label, check.url, check.status]),
  ]
...
function ServiceHealthSummary({
  ...
  return (
    <article className={`vx-service-health-summary__item vx-service-health-status--${status}`}>
...
function ServiceHealthCard({ service }: { service: DevServiceSnapshot }) {
  ...
  return (
    <article className={`vx-service-health-card vx-service-health-status--${status}`}>
      ...
      <div className="vx-service-health-card__checks" aria-label={`${service.name} 探针`}>
      ...
      <footer className="vx-service-health-card__footer">
        ...
        <strong>{failedChecks ? `${failedChecks} 个异常探针` : `${service.health.length} 个探针通过`}</strong>
      </footer>
    </article>
  );
}
...
export function ServiceHealthPage({
  ...
  const visibleServices = useMemo(() => {
    ...
    return services
      .filter((service) => {
        ...
        const matchesQuery = !normalizedQuery || serviceSearchText(service).includes(normalizedQuery);
      ...
      .sort((left, right) => {
        const statusDiff = statusOrder(serviceStatus(left)) - statusOrder(serviceStatus(right));
  ...
  const stats = useMemo(() => {
    const healthy = services.filter((service) => serviceStatus(service) === 'healthy').length;
    const degraded = services.filter((service) => serviceStatus(service) === 'degraded').length;
    const offline = services.filter((service) => serviceStatus(service) === 'offline').length;
    const failedChecks = services.reduce((total, service) => total + service.health.filter((check) => !check.ok).length, 0);
  ...
  return (
    <div className="vx-page-stack vx-service-health-page">
      ...
      <section className="vx-service-health-summary" aria-label="服务健康概览">
        <ServiceHealthSummary label="服务总数" value={String(stats.total)} tag="" icon="server" status="healthy" />
        <ServiceHealthSummary label="服务正常" value={String(stats.healthy)} tag="" icon="success" status="healthy" />
        <ServiceHealthSummary label="服务启动，探针异常" value={String(stats.degraded)} tag="" icon="warning" status="degraded" />
        <ServiceHealthSummary label="服务未启动" value={String(stats.offline)} tag="" icon="error" status="offline" />
      </section>
      ...
      {visibleServices.length ? (
        ...
        <section className="vx-service-health-empty">
          <EmptyState
            ...
            description={loading ? '正在读取 dev-tools 只读监测结果。' : '调整筛选条件，或重置后查看全部服务。'}
            action={
