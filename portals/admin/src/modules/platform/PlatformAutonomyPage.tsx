...
type AutonomyMetric = { label: string; value: string; detail: string; icon: IconName; tone: Tone };
type ResourceRow = { subject: string; key: string; model: string; quota: string; usage: string; status: string };
...
const autonomyDomains = [
  {
    ...
    links: [
      ...
      { label: '权限策略', href: '/admin-permissions', meta: '权限点、层级、绑定' },
    ],
  },
  {
    title: '平台资源',
    ...
    icon: 'cloud',
    links: [
      { label: '模型网关', href: '/model-gateway', meta: 'Provider、端点、链路' },
      { label: '密钥管理', href: '/platform-secrets', meta: '凭据、轮换、可见性' },
    ],
  },
  {
    title: '运行保障',
...
function buildResourceRows(models: AiModelRecord[], grants: AiModelGrantRecord[]): ResourceRow[] {
...
export function PlatformAutonomyPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchAiModels(true), fetchAiModelGrants()])
      ...
      .catch((error: unknown) => {
        ...
        setModels([]);
        setGrants([]);
        setResourceError(error instanceof Error ? error.message : 'AI Gateway 数据读取失败');
      })
      .finally(() => {
  ...
  const resourceRows = useMemo(() => buildResourceRows(models, grants), [models, grants]);
          ...
          <div className="platform-autonomy-resource-table">
            ...
            {loadingResources ? (
              ...
              resourceRows.map((row) => (
                <div key={`${row.key}-${row.model}`} className="platform-autonomy-resource-row">
