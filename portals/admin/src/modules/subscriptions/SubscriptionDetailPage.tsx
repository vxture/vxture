...
import { fetchSubscriptionOperation, submitSubscriptionOperation } from '@/api/admin-bff';
import type {
...
import { formatDate, formatMoney, formatNumber, typeLabel } from '@/modules/tenants/tenant-utils';
...
function associationSourceLabel(source: SubscriptionOperationDetailRecord['solutionAssociation']['source']) {
...
function SubscriptionSummary({ subscription }: { subscription: SubscriptionOperationDetailRecord }) {
  return (
    <section className="vx-product-capability-summary">
      <div className="vx-product-capability-summary__identity">
        ...
        <div>
          <h2>{subscription.tenantName} / {subscription.tierName}</h2>
      ...
      <div className="vx-product-capability-summary__metrics">
        <DetailMetric label="业务方案" value={subscription.solutionAssociation.solutionName} tag={associationSourceLabel(subscription.solutionAssociation.source)} />
        <DetailMetric label="月收入" value={formatMoney(subscription.monthlyRevenue)} tag={cycleLabel(subscription.cycleType)} />
        <DetailMetric label="配额消耗" value={`${formatNumber(subscription.quota.usageRate)}%`} tag={`${formatNumber(subscription.quota.maxUsers)} 席位`} />
        <DetailMetric label="运营动作" value={subscription.operationHint} tag={subscription.autoRenew ? '自动续期' : '人工跟进'} />
      </div>
    </section>
  );
}

function SubscriptionDetails({ subscription }: { subscription: SubscriptionOperationDetailRecord }) {
  ...
  return (
    <section className="vx-product-capability-detail" aria-label={`${subscription.tenantName} 订阅详情`}>
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          ...
          <DetailField label="订单编号" value={subscription.orderNo ?? '未设置'} />
          <DetailField label="租户" value={subscription.tenantName} />
          <DetailField label="租户类型" value={typeLabel(subscription.tenantType)} />
          <DetailField label="订阅状态" value={subscriptionStatusLabel(subscription.status)} />
          <DetailField label="计费周期" value={cycleLabel(subscription.cycleType)} />
          <DetailField label="自动续期" value={subscription.autoRenew ? '是' : '否'} />
          <DetailField label="运营创建人" value={subscription.operatorName} />
          <DetailField label="开通时间" value={formatDate(subscription.startAt)} />
          <DetailField label="到期时间" value={formatDate(subscription.endAt)} />
          <DetailField label="试用结束" value={formatDate(subscription.trialEndAt)} />
          <DetailField label="更新时间" value={formatDate(subscription.updatedAt)} />
        </div>
      </section>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          <DetailField label="业务方案" value={subscription.solutionAssociation.solutionName} />
          <DetailField label="方案编码" value={subscription.solutionAssociation.solutionCode ?? '未显式绑定'} />
          <DetailField label="套餐层级" value={subscription.solutionAssociation.tierName} />
          <DetailField label="关联来源" value={associationSourceLabel(subscription.solutionAssociation.source)} />
        </div>
        ...
        <div className="vx-product-capability-actions vx-subscription-detail-links">
          {subscription.solutionAssociation.solutionCode ? (
            <Link href={`/product-solutions/${encodeURIComponent(subscription.solutionAssociation.solutionCode)}`} className="vx-btn vx-btn--outline vx-btn--default">
              <Icon name="workflow" size="xs" fallback="placeholder" />
              业务方案
            </Link>
          ) : null}
          {servicePlanHref ? (
            <Link href={servicePlanHref} className="vx-btn vx-btn--outline vx-btn--default">
        ...
        <div className="vx-product-detail-list vx-product-detail-list--entitlements">
          {subscription.entitlementSnapshot.map((item) => (
            <div key={item.productCode} className="vx-product-detail-list__row">
              <span>
                <Icon name={capabilityTypeIcon(item.productType)} size="sm" fallback="placeholder" />
                <strong>{item.productName}</strong>
              </span>
              <small>{capabilityTypeLabel(item.productType)} | {item.source === 'self' ? '自建' : '三方'}</small>
              <em className={item.included ? 'is-included' : 'is-excluded'}>{item.included ? '包含' : '不包含'} | {item.quotaSummary}</em>
              <p>{item.note}</p>
            </div>
          ))}
        </div>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          <DetailField label="最大席位" value={`${formatNumber(subscription.quota.maxUsers)} 人`} />
          <DetailField label="Token 配额" value={formatNumber(subscription.quota.periodTokens)} />
          <DetailField label="已消耗 Token" value={formatNumber(subscription.quota.usedTokens)} />
          <DetailField label="消耗比例" value={`${formatNumber(subscription.quota.usageRate)}%`} />
          <DetailField label="配额周期" value={cycleLabel(subscription.quota.quotaCycle)} />
          <DetailField label="允许模型" value={`${formatNumber(subscription.quota.allowedModelCount)} 个`} />
          <DetailField label="自定义模型" value={subscription.quota.allowCustomModel ? '允许' : '不允许'} />
          <DetailField label="配额风险" value={quotaRiskLabel(subscription.quota.risk)} />
        </div>
      </section>
...
export function SubscriptionDetailPage({ subscriptionId }: { subscriptionId: string }) {
  const [subscription, setSubscription] = useState<SubscriptionOperationDetailRecord | null>(null);
  const [loading, setLoading] = useState(true);
  const [pendingAction, setPendingAction] = useState<SubscriptionOperationAction | null>(null);
  ...
  async function handleSubmitSubscriptionAction(reason: string) {
    ...
    try {
      const updatedSubscription = await submitSubscriptionOperation(subscription.id, {
    ...
    return (
      <div className="vx-page-stack vx-product-capability-page">
        <PageHeader
          ...
          action={
            <Link href="/subscriptions" className="vx-btn vx-btn--outline vx-btn--default">
              <Icon name="arrow-left" size="xs" fallback="placeholder" />
              返回列表
            </Link>
          }
        />
        <EmptyState title="订阅实例不存在" description="该订阅可能已归档，或当前账号无权访问。" />
      </div>
    );
  }
  ...
  return (
    <div className="vx-page-stack vx-product-capability-page vx-subscription-detail-page">
      <PageHeader
        icon="star"
        title={subscription ? `${subscription.tenantName} / ${subscription.tierName}` : '订阅详情'}
        description={subscription?.solutionAssociation.note ?? '正在读取租户订阅权益实例。'}
        action={
          <div className="vx-product-capability-actions">
            ...
            {subscription ? (
              <Link href={`/tenants/${encodeURIComponent(subscription.tenantId)}`} className="vx-btn vx-btn--outline vx-btn--default">
            ...
            {subscription ? (
              <>
                ...
                <Button
                  variant="outline"
                  onClick={() => requestSubscriptionAction(subscriptionToggleAction(subscription.status))}
                  disabled={!canRunSubscriptionAction(subscriptionToggleAction(subscription.status), subscription)}
                  title={subscriptionActionDisabledReason(subscriptionToggleAction(subscription.status), subscription) ?? undefined}
                >
                  <Icon name={subscriptionActionIcon(subscriptionToggleAction(subscription.status))} size="xs" fallback="placeholder" />
                  {subscriptionActionLabel(subscriptionToggleAction(subscription.status))}
                </Button>
                <Button
                  variant="outline"
