...
type SubscriptionActionTarget = SubscriptionOperationStatus | {
...
export function subscriptionActionIcon(action: SubscriptionOperationAction): IconName {
...
export function subscriptionToggleAction(status: SubscriptionOperationStatus): SubscriptionOperationAction {
  return status === 'suspended' ? 'resume' : 'suspend';
}

export function subscriptionActionDisabledReason(action: SubscriptionOperationAction, target: SubscriptionActionTarget): string | null {
...
export function canRunSubscriptionAction(action: SubscriptionOperationAction, target: SubscriptionActionTarget) {
...
function subscriptionActionDescription(action: SubscriptionOperationAction) {
  if (action === 'renew') return '确认合同、付款或续约审批已生效。系统会延长当前周期；临期、逾期、暂停订阅会重新进入已生效状态。';
  if (action === 'suspend') return '暂停用于账务、合规或运营风险冻结。暂停后将关闭自动续期，后续可恢复或续期确认。';
  if (action === 'resume') return '恢复仅用于仍在有效期内的暂停订阅。若订阅已过期，应先做续期确认。';
...
function subscriptionActionPlaceholder(action: SubscriptionOperationAction) {
  ...
  if (action === 'suspend') return '例如：合同付款未确认，暂停权益等待运营复核。';
...
export function SubscriptionOperationDialog({
  ...
  return (
    <div className="vx-subscription-action-dialog" role="dialog" aria-modal="true" aria-labelledby="subscription-action-title">
      <form
      ...
      >
        <header>
          <span aria-hidden="true" className={`vx-subscription-action-dialog__icon vx-subscription-action-dialog__icon--${action}`}>
            <Icon name={subscriptionActionIcon(action)} size="lg" fallback="placeholder" />
          </span>
          <div>
            <h2 id="subscription-action-title">{subscriptionActionLabel(action)}</h2>
        ...
        <footer>
          ...
          <Button type="submit" className={action === 'cancel' ? 'vx-subscription-action-button--danger' : undefined} disabled={busy || !trimmedReason}>
