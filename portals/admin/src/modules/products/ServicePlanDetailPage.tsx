...
function ServicePlanSummary({ plan }: { plan: ProductServicePlanDetailRecord }) {
  return (
    <section className="vx-product-capability-summary">
      <div className="vx-product-capability-summary__identity">
        ...
        <div>
          <h2>{plan.solutionName} / {plan.tierName}</h2>
          <p>{plan.solutionCode} · {plan.tierCode}</p>
          <div className="vx-product-capability-summary__badges">
            <Badge className={`vx-tenant-pill vx-service-plan-pill--${plan.status}`}>{statusLabel(plan.status)}</Badge>
            <Badge className={`vx-tenant-pill vx-service-plan-pill--${plan.isPublic ? 'public' : 'internal'}`}>{plan.isPublic ? '公开' : '内部'}</Badge>
          </div>
        </div>
      </div>
      <div className="vx-product-capability-summary__metrics">
        <DetailMetric label="价格" value={plan.price.priceLabel} tag={plan.price.periodType === 'contract' ? '专属商务' : '标准价格'} />
        <DetailMetric label="包含产品" value={formatNumber(plan.includedProductCount)} tag={`不含 ${formatNumber(plan.excludedProductCount)}`} />
        <DetailMetric label="订阅使用" value={formatNumber(plan.subscriptionCount)} tag={`活跃 ${formatNumber(plan.activeTenantCount)}`} />
        <DetailMetric label="适用范围" value={formatNumber(plan.applicableScope.length)} tag={plan.industry} />
      </div>
    </section>
  );
}

function ServicePlanDetails({ plan }: { plan: ProductServicePlanDetailRecord }) {
  return (
    <section className="vx-product-capability-detail" aria-label={`${plan.solutionName} ${plan.tierName} 详情`}>
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          ...
          <DetailField label="可见范围" value={plan.isPublic ? '公开' : '内部'} />
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          ...
          <DetailField label="周期" value={plan.price.periodType === 'contract' ? '合同约定' : plan.price.periodType === 'yearly' ? '年付' : '月付'} />
          <DetailField label="订阅数量" value={`${formatNumber(plan.subscriptionCount)} 个`} />
          <DetailField label="活跃租户" value={`${formatNumber(plan.activeTenantCount)} 个`} />
        </div>
      </section>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-detail-list vx-product-detail-list--entitlements">
          {plan.entitlements.map((item) => (
            <Link key={item.productCode} href={`/products/${encodeURIComponent(item.productCode)}`} className="vx-product-detail-list__row">
              <span>
...
export function ServicePlanDetailPage({ solutionCode, tierCode }: { solutionCode: string; tierCode: string }) {
    ...
    return (
      <div className="vx-page-stack vx-product-capability-page">
        <PageHeader
          ...
          action={
            <Link href="/service-plans" className="vx-btn vx-btn--outline vx-btn--default">
              <Icon name="arrow-left" size="xs" fallback="placeholder" />
              返回列表
            </Link>
          }
        />
        <EmptyState title="服务套餐不存在" description="该套餐可能已归档，或当前账号无权访问。" />
      </div>
    );
  }
  ...
  return (
    <div className="vx-page-stack vx-product-capability-page">
      <PageHeader
        icon="star"
        title={plan ? `${plan.solutionName} / ${plan.tierName}` : '服务套餐详情'}
        ...
        action={
          <div className="vx-product-capability-actions">
            ...
            {plan ? (
              <Link href={`/product-solutions/${encodeURIComponent(plan.solutionCode)}`} className="vx-btn vx-btn--outline vx-btn--default">
