...
function ProductSectionHeading({ icon, title }: { icon: IconName; title: string }) {
  return (
...
function ProductCapabilitySummary({ product }: { product: ProductCapabilityRecord }) {
  return (
    <section className="vx-product-capability-summary">
      <div className="vx-product-capability-summary__identity">
        <span className="vx-product-capability-summary__icon" aria-hidden="true">
          <Icon name={capabilityTypeIcon(product.productType)} size="lg" fallback="placeholder" />
        </span>
        <div>
          <h2>{product.productName}</h2>
          <p>{product.productCode}</p>
          <div className="vx-product-capability-summary__badges">
            <Badge className={`vx-tenant-pill vx-product-pill--${product.productType}`}>{capabilityTypeLabel(product.productType)}</Badge>
            <Badge className={`vx-tenant-pill vx-product-pill--${product.source}`}>{sourceLabel(product.source)}</Badge>
            <Badge className={`vx-tenant-pill vx-product-pill--${product.status}`}>{statusLabel(product.status)}</Badge>
          </div>
        </div>
      </div>
...
function ProductCapabilityDetails({ product }: { product: ProductCapabilityRecord }) {
  return (
    <section className="vx-product-capability-detail" aria-label={`${product.productName} 产品能力详情`}>
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          ...
          <DetailField label="产品类型" value={capabilityTypeLabel(product.productType)} />
          <DetailField label="产品来源" value={sourceLabel(product.source)} />
          <DetailField label="可见范围" value={product.visibility === 'public' ? '公开' : '内部'} />
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-tags">
          {product.accessModes.map((mode) => <Badge key={mode} className="vx-tenant-pill vx-product-capability-pill--mode">{mode}</Badge>)}
          {product.tags.map((tag) => <Badge key={tag} className="vx-tenant-pill vx-product-capability-pill--tag">{tag}</Badge>)}
        </div>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          <DetailField label="供应商" value={product.integration.providerName} />
          <DetailField label="供应商类型" value={sourceLabel(product.integration.providerType)} />
          <DetailField label="接入状态" value={integrationStatusLabel(product.integration.status)} />
          <DetailField label="协议" value={product.integration.protocol} />
          <DetailField label="认证方式" value={product.integration.authMode} />
          <DetailField label="结算方式" value={product.integration.settlementMode ?? '无'} />
          <DetailField label="接口地址" value={product.integration.endpoint ?? '内部能力，无需外部接口'} />
          <DetailField label="最近检测" value={product.integration.lastCheckedAt ? formatDate(product.integration.lastCheckedAt) : '未检测'} />
        </div>
      </section>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields vx-product-capability-fields--compact">
          ...
          <DetailField label="策略数量" value={`${formatNumber(product.modelPolicyCount)} 个`} />
        </div>
        <div className="vx-product-capability-metric-rules">
          {product.metrics.map((metric) => (
            <article key={metric.metricCode}>
              ...
              <small>{metric.unit} | {metric.cycle} | {metric.quotaBase} | {metric.billingMode}</small>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="vx-product-capability-section">
        ...
        <div className="vx-product-capability-fields">
          ...
          <DetailField label="健康状态" value={healthLabel(product.healthStatus)} />
          <DetailField label="发布数量" value={`${formatNumber(product.releaseCount)} 个`} />
          <DetailField label="方案复用" value={`${formatNumber(product.solutionCount)} 个`} />
        </div>
        <div className="vx-product-capability-related-list">
...
export function ProductCapabilityDetailPage({ productCode }: { productCode: string }) {
    ...
    return (
      <div className="vx-page-stack vx-product-capability-page">
        <PageHeader
          ...
          action={
            <Link href="/products" className="vx-btn vx-btn--outline vx-btn--default">
