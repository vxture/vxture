...
function productSearchText(product: ProductCapabilityRecord) {
  return [
    ...
    ...product.tags,
    ...product.relatedSolutions.map((solution) => `${solution.solutionCode} ${solution.solutionName} ${solution.role}`),
    ...product.releases.map((release) => `${release.releaseCode} ${release.releaseName} ${release.versionLabels.join(' ')}`),
  ].join(' ').toLowerCase();
}
...
function ProductPageSizePicker({ value, onChange }: { value: PageSize; onChange: (value: PageSize) => void }) {
  return (
        ...
        >
          ...
          <span className="vx-product-row__type">
            <span className="vx-tenant-directory-row__tag-line">
              <Badge className={`vx-tenant-pill vx-product-pill--${product.productType}`}>{productTypeLabel(product.productType)}</Badge>
              <Badge className={`vx-tenant-pill vx-product-pill--${product.source}`}>{productSourceLabel(product.source)}</Badge>
            </span>
          </span>
          <span className="vx-product-row__status">
            <Badge className={`vx-tenant-pill vx-product-pill--${product.status}`}>{productStatusLabel(product.status)}</Badge>
            <small>{product.visibility === 'public' ? '公开' : '内部'} | {product.healthStatus === 'normal' ? '健康' : '关注'}</small>
          </span>
          <span className="vx-product-row__supply">
            <strong>{formatNumber(product.solutionCount)} 方案</strong>
            <small>{formatNumber(product.planCount)} 套餐 | {formatNumber(product.releaseCount)} 发布</small>
          </span>
...
function ProductCards({
  ...
  return (
    <div className="vx-tenant-directory-cards vx-product-cards" aria-label="产品能力卡片">
      {products.map((product) => (
        <article
          key={product.productCode}
          className={joinClasses('vx-tenant-directory-card', `vx-product-card--${product.status}`)}
          role="button"
          tabIndex={0}
        ...
        >
          <header>
            <Icon name={productTypeIcon(product.productType)} size="lg" fallback="placeholder" />
            <div>
              <strong>{product.productName}</strong>
              <span>{product.productCode} · {productRegionLabel(product.region)}</span>
            </div>
            <ProductActionsMenu
              product={product}
              open={openMenuId === product.productCode}
...
export function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      if (accessFilter !== 'all' && product.integration.status !== accessFilter) return false;
      if (normalizedQuery && !productSearchText(product).includes(normalizedQuery)) return false;
      return true;
    });
  ...
  const partnerProducts = products.filter((product) => product.source === 'partner').length;
  const solutionCount = new Set(products.flatMap((product) => product.relatedSolutions.map((solution) => solution.solutionCode))).size;
  const configRequiredProducts = products.filter((product) => product.integration.status === 'config_required' || product.integration.status === 'testing').length;
  ...
  return (
    <div className="vx-page-stack vx-tenant-management-page vx-product-management-page">
      ...
      <section className="vx-tenant-summary" aria-label="产品能力管理统计">
        <ProductSummaryItem icon="database" label="能力总数" value={formatNumber(products.length)} tags={[`上线 ${formatNumber(activeProducts)}`]} />
        <ProductSummaryItem icon="agent" label="能力类型" value={formatNumber(agentProducts + platformProducts)} tags={[`智能体 ${formatNumber(agentProducts)}`, `平台 ${formatNumber(platformProducts)}`]} tone="green" />
        <ProductSummaryItem icon="cloud" label="三方接入" value={formatNumber(partnerProducts)} tags={['合作方']} tone={partnerProducts ? 'amber' : 'green'} />
        <ProductSummaryItem icon="workflow" label="方案复用" value={formatNumber(solutionCount)} tags={[`待配置 ${formatNumber(configRequiredProducts)}`]} tone={configRequiredProducts ? 'amber' : 'blue'} />
      </section>

      <div className="vx-tenant-list-shell">
        <section className="vx-tenant-toolbar" aria-label="产品能力筛选">
          <ViewModeSwitch value={viewMode} onChange={setViewMode} ariaLabel="产品能力展示方式" />
          <span className="vx-tenant-view-count">{formatNumber(filteredProducts.length)}</span>
          ...
          {visibleProducts.length ? (
            ...
            <section className="vx-tenant-empty">
              <EmptyState
                ...
                description={loading ? '正在读取产品能力供给目录。' : '清空筛选条件后可查看全部产品能力。'}
                action={
