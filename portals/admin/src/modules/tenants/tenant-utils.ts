...
export function normalizeTenantRiskLevel(risk: TenantRiskValue): TenantOperationRecord['riskLevel'] {
...
export function subscriptionStatusLabel(status: TenantOperationSubscription['status']) {
...
export function modelPolicyStateLabel(state: TenantOperationModelPolicy['state']) {
...
export function policySourceLabel(source: TenantOperationModelPolicy['source']) {
...
export function tenantSearchText(tenant: TenantOperationRecord) {
  return [
    ...
    tenant.riskLevel,
    ...tenant.tags,
    ...tenant.subscriptions.map((item) => `${item.productName} ${item.releaseName} ${item.planName}`),
  ].join(' ').toLowerCase();
}
