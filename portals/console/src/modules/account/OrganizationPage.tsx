...
function formatOrganizationDate(value: string | null | undefined, locale: string, fallback: string) {
  if (!value) {
    return fallback;
  }
...
function logoStyle(value?: string | null): CSSProperties | undefined {
  ...
  return logoUrl ? { backgroundImage: `url(${JSON.stringify(logoUrl)})` } : undefined;
}
...
function readLogoOverride(tenantId?: string | null): LogoOverride | null {
  try {
    const stored = window.localStorage.getItem(organizationLogoStorageKey(tenantId));
...
function writeLogoOverride(tenantId: string | undefined, value: LogoOverride) {
  window.localStorage.setItem(organizationLogoStorageKey(tenantId), JSON.stringify(value));
}
...
export function OrganizationPage() {
  const t = useConsoleTranslations('organizationPage');
  ...
  const displayName = displayValue(profile?.displayName, profile?.companyName || profile?.tenantName || session.tenant?.name || empty);
  const logoInitials = organizationInitials(displayName || 'VX');
  const effectiveLogoUrl = logoOverride ? logoOverride.logoUrl : profile?.logoUrl;
  const location = [profile?.countryCode, profile?.province, profile?.city, profile?.district].filter(Boolean).join(' / ');
  const status = profile ? t(`status.tenant.${profile.status}`) : empty;
  const verifiedStatus = profile?.verifiedStatus ? t(`status.verified.${profile.verifiedStatus}`) : empty;
  const updatedAt = formatOrganizationDate(logoOverride?.updatedAt ?? profile?.updatedAt, locale, empty);
  ...
  const logoStatus = effectiveLogoUrl ? t('summary.customLogo') : t('summary.defaultLogo');
  const industryScale = [profile?.industry, profile?.scale].filter(Boolean).join(' / ') || empty;

  const foundationFields: OrganizationField[] = [
    { icon: 'building-library', label: t('fields.organizationName'), value: loading ? loadingText : displayName },
    { icon: 'medal', label: t('fields.companyName'), value: loading ? loadingText : displayValue(profile?.companyName, empty) },
    { icon: 'key', label: t('fields.tenantCode'), value: loading ? loadingText : displayValue(profile?.tenantCode, empty) },
    { icon: 'users', label: t('fields.tenantType'), value: loading ? loadingText : tenantType },
    { icon: 'table', label: t('fields.industryScale'), value: loading ? loadingText : industryScale },
    { icon: 'chat-circle', label: t('fields.description'), value: loading ? loadingText : displayValue(profile?.description, empty), multiline: true },
  ];

  const statusFields: OrganizationField[] = [
    { icon: 'server', label: t('fields.tenantStatus'), value: loading ? loadingText : status },
    { icon: 'shield-check', label: t('fields.verifiedStatus'), value: loading ? loadingText : verifiedStatus },
    {
      icon: 'key',
      label: t('fields.unifiedSocialCreditCode'),
      value: loading ? loadingText : displayValue(profile?.unifiedSocialCreditCode, empty),
    },
    { icon: 'clock', label: t('fields.verifiedAt'), value: loading ? loadingText : verifiedAt },
    { icon: 'warning', label: t('fields.rejectedReason'), value: loading ? loadingText : displayValue(profile?.rejectedReason, empty) },
  ];

  const contactFields: OrganizationField[] = [
    { icon: 'user', label: t('fields.contactName'), value: loading ? loadingText : displayValue(profile?.contactName, empty) },
    { icon: 'phone', label: t('fields.contactPhone'), value: loading ? loadingText : displayValue(profile?.contactPhone, empty) },
    { icon: 'mail', label: t('fields.contactEmail'), value: loading ? loadingText : displayValue(profile?.contactEmail, empty) },
    { icon: 'globe', label: t('fields.primaryDomain'), value: loading ? loadingText : displayValue(profile?.primaryDomain, empty) },
    { icon: 'map-pin', label: t('fields.location'), value: loading ? loadingText : displayValue(location, empty) },
    { icon: 'map-pin', label: t('fields.address'), value: loading ? loadingText : displayValue(profile?.address, empty), multiline: true },
    { icon: 'mail', label: t('fields.postalCode'), value: loading ? loadingText : displayValue(profile?.postalCode, empty) },
  ];

  const localizationFields: OrganizationField[] = [
    { icon: 'globe', label: t('fields.language'), value: loading ? loadingText : displayValue(profile?.language, empty) },
    { icon: 'clock', label: t('fields.timeZone'), value: loading ? loadingText : displayValue(profile?.timeZone, empty) },
    {
      icon: 'database',
      label: t('fields.businessLicenseUrl'),
      value: loading ? loadingText : displayValue(profile?.businessLicenseUrl, empty),
      multiline: true,
    },
  ...
  return (
    <div className="vx-page-stack vx-profile-page vx-account-profile-page vx-organization-profile-page">
      ...
      {messageKey ? <p className="vx-profile-message">{t(messageKey)}</p> : null}
      ...
      <div className="vx-account-profile-layout">
        <aside className="vx-account-profile-identity">
          <section className="vx-account-profile-avatar-card vx-organization-profile-card">
            ...
            >
              <Avatar
                className={effectiveLogoUrl ? 'vx-organization-logo vx-organization-logo--image' : 'vx-organization-logo'}
                role="img"
                aria-label={t('logo.alt', { name: displayName })}
                style={logoStyle(effectiveLogoUrl)}
              >
                {!effectiveLogoUrl ? <strong>{logoInitials}</strong> : <span className="vx-account-profile-avatar__label">{displayName}</span>}
              </Avatar>
            ...
            <div className="vx-account-profile-avatar-card__copy">
              ...
              <span>{profile?.tenantCode || session.tenant?.tenantCode || empty}</span>
              <Badge className={`vx-organization-status vx-organization-status--${profile?.status ?? 'trial'}`}>
            ...
            <div className="vx-account-profile-avatar-card__actions">
              ...
              <ActionButton variant="outline" icon="x" size="sm" onClick={clearLogo} disabled={!effectiveLogoUrl}>
                {t('actions.clearLogo')}
              </ActionButton>
            </div>
          </section>
          ...
          <section className="vx-account-profile-summary">
            <div className="vx-account-profile-summary__row">
              ...
              <strong title={displayName}>{loading ? loadingText : displayName}</strong>
            </div>
            <div className="vx-account-profile-summary__row">
            ...
            <div className="vx-account-profile-summary__row">
              ...
              <strong title={profile?.tenantName || session.tenant?.name || empty}>
                {loading ? loadingText : displayValue(profile?.tenantName, session.tenant?.name || empty)}
              </strong>
            </div>
            <div className="vx-account-profile-summary__row">
            ...
            <div className="vx-account-profile-summary__row">
              ...
              <strong title={updatedAt}>{loading ? loadingText : updatedAt}</strong>
            </div>
          </section>
        </aside>
        ...
        <main className="vx-account-profile-main">
          <section className="vx-account-profile-section">
            ...
            <div className="vx-account-profile-field-list">{foundationFields.map(renderField)}</div>
          </section>
          ...
          <section className="vx-account-profile-section">
            ...
            <div className="vx-account-profile-field-list">{statusFields.map(renderField)}</div>
          </section>
          ...
          <section className="vx-account-profile-section">
            ...
            <div className="vx-account-profile-field-list">{localizationFields.map(renderField)}</div>
          </section>
        </main>
      </div>

      {logoDialogOpen ? (
        <div className="vx-profile-dialog" role="dialog" aria-modal="true" aria-label={t('dialogs.logo.title')}>
          <div className="vx-profile-dialog__backdrop" onClick={() => setLogoDialogOpen(false)} />
          <form
          ...
          >
            ...
            <div className="vx-account-profile-dialog-grid">
              <div className="vx-account-avatar-editor vx-organization-logo-editor">
                <Avatar
                  className={logoForm.logoUrl ? 'vx-organization-logo vx-organization-logo--image' : 'vx-organization-logo'}
                  role="img"
                  aria-label={t('logo.previewAlt', { name: displayName })}
                  style={logoStyle(logoForm.logoUrl)}
                >
                  {!logoForm.logoUrl ? <strong>{logoInitials}</strong> : <span className="vx-account-profile-avatar__label">{displayName}</span>}
                </Avatar>
                <div>
                  <strong>{displayName}</strong>
                  <span>{logoForm.logoUrl ? t('logo.customPreview') : t('logo.defaultPreview')}</span>
                </div>
              </div>
              ...
              <div className="vx-account-profile-form-grid">
                <Label className="vx-account-profile-form-grid__wide">
                  ...
                  <div className="vx-account-profile-input-row">
                    <Input
                      ...
                      onChange={(event) => setLogoForm({ logoUrl: event.target.value })}
                      placeholder={t('placeholders.logoUrl')}
                      autoComplete="url"
                    />
                    <Button variant="outline" onClick={() => setLogoForm({ logoUrl: '' })}>
                      {t('actions.clear')}
                    </Button>
                  </div>
                </Label>
                <div className="vx-organization-logo-upload vx-account-profile-form-grid__wide">
                  ...
                  <Button variant="outline" onClick={() => logoFileInputRef.current?.click()}>
                    <Icon name="plus" size="xs" fallback="placeholder" className="vx-btn__icon" />
            ...
            <div className="vx-profile-dialog__actions">
              ...
              <Button variant="outline" onClick={clearLogo} disabled={!logoForm.logoUrl && !effectiveLogoUrl}>
                {t('actions.clearLogo')}
              </Button>
