...
type TopicKey = 'account' | 'security' | 'subscription' | 'billing' | 'usage' | 'product';
  ...
  try {
    ...
    return {
      topics: DEFAULT_NOTIFICATION_STATE.topics.map((topic) => {
        const storedTopic = storedTopics.find((item) => item?.key === topic.key);
        const storedChannels =
          storedTopic && typeof storedTopic.channels === 'object' && storedTopic.channels !== null ? storedTopic.channels : {};
...
export function NotificationsPage() {
  ...
  const enabledTopics = state.topics.filter((topic) => CHANNELS.some((channel) => topic.channels[channel.key])).length;
  const emailTopics = state.topics.filter((topic) => topic.channels.email).length;
  ...
  function toggleTopicChannel(topicKey: TopicKey, channelKey: ChannelKey, enabled: boolean) {
    setState((current) => ({
      topics: current.topics.map((topic) => {
        if (topic.key !== topicKey || topic.lockedChannels?.includes(channelKey)) {
          return topic;
        }
  ...
  return (
    <div className="vx-page-stack vx-notifications-page">
      ...
      <section className="vx-notification-preferences">
        ...
        <div className="vx-notification-preferences__stats" aria-label={t('summary.title')}>
          <div>
          ...
          <div>
            ...
            <strong>{t('summary.topicsValue', { enabled: enabledTopics, total: totalTopics })}</strong>
          </div>
        </div>
      </section>
      ...
      <section className="vx-notification-board">
        ...
        <div className="vx-notification-groups">
          {TOPIC_GROUPS.map((group) => {
            const groupTopics = state.topics.filter((topic) => topic.group === group.key);
              ...
              <section key={group.key} className="vx-notification-group">
                <div className="vx-notification-group__title">
                  ...
                  <div>
                    ...
                    <span>{t('topics.groupCount', { count: groupTopics.length })}</span>
                  </div>
                </div>
                ...
                <div className="vx-notification-table">
                  ...
                  {groupTopics.map((topic) => {
                    ...
                    return (
                      <div key={topic.key} className="vx-notification-table__row">
                        <div className="vx-notification-topic-cell">
                          <span aria-hidden="true">
                            <Icon name={topic.icon} size="xs" fallback="placeholder" />
                          </span>
                          <strong>{t(`topics.items.${topic.key}.title`)}</strong>
                          {locked ? <Badge>{t('topics.policyLocked')}</Badge> : null}
                        </div>

                        {CHANNELS.map((channel) => {
                          const channelLocked = topic.lockedChannels?.includes(channel.key) ?? false;
                        ...
                        <span className={enabled ? 'vx-notification-status vx-notification-status--on' : 'vx-notification-status'}>
