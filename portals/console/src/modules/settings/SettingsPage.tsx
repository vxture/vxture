...
type BooleanSettingKey = 'inviteApproval' | 'externalMembers' | 'mfaRequired' | 'trustedDeviceReview' | 'exportApproval' | 'apiKeyCreation';
...
type TenantSettingState = Record<BooleanSettingKey, boolean> & Record<SelectSettingKey, string>;
...
const SETTING_SECTIONS: SettingSection[] = [
  {
    ...
    rows: [
      ...
      { key: 'defaultRole', icon: 'shield-check', type: 'select', options: ['readonly', 'member', 'admin'] },
    ],
  },
  {
    key: 'security',
    ...
    rows: [
      ...
      { key: 'sessionTimeout', icon: 'clock', type: 'select', options: ['8h', '24h', '7d'] },
    ],
  },
  {
    key: 'data',
    icon: 'database',
    rows: [
      { key: 'auditRetention', icon: 'clock', type: 'select', options: ['90', '180', '365'] },
...
export function SettingsPage() {
  ...
  const enabledPolicies = (Object.keys(DEFAULT_TENANT_SETTINGS) as SettingKey[]).filter((key) => {
    const value = settings[key];
    return typeof value === 'boolean' ? value : value !== DEFAULT_TENANT_SETTINGS[key];
  }).length;
  ...
  return (
    <div className="vx-page-stack vx-tenant-settings-page">
      <div className="vx-tenant-settings-title">
        ...
        <section className="vx-tenant-settings-summary" aria-label={t('summary.title')}>
          ...
          <div className="vx-tenant-settings-summary__item">
            ...
            <strong>{t('summary.enabledValue', { count: enabledPolicies })}</strong>
          </div>
          <div className="vx-tenant-settings-summary__item">
      ...
      <div className="vx-tenant-settings-general">
        <header className="vx-tenant-settings-block-header">
          <div>
            ...
            <span>{t('general.count', { count: SETTING_SECTIONS.length })}</span>
          </div>
        </header>
          ...
          <section key={section.key} className="vx-tenant-settings-section">
            <header className="vx-tenant-settings-section__title">
              ...
              <div>
                ...
                <span>{t(`sections.${section.key}.count`, { count: section.rows.length })}</span>
              </div>
            </header>
                  ...
                  <div key={row.key} className="vx-tenant-settings-row">
                    <div className="vx-tenant-settings-row__policy">
                      <span aria-hidden="true">
                        <Icon name={row.icon} size="xs" fallback="placeholder" />
                      </span>
                      <div>
                    ...
                    <div className="vx-tenant-settings-control">
                      {row.type === 'switch' ? (
                        ...
                        <NativeSelect
                          ...
                          aria-label={t(`rows.${row.key}.title`)}
                          onChange={(event) => updateSelectSetting(row.key as SelectSettingKey, event.target.value)}
                        >
