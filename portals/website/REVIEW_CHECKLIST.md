...
- [ ] **C-07** 检查 `messages/` 目录结构：顶层目录必须是 `zh-CN/` 和 `en-US/`。

- [ ] **C-08** 检查 `messages/zh-CN/` 和 `messages/en-US/` 下的文件，确认翻译按 namespace 拆分为多个 JSON 文件（`common.json`、`nav.json` 等），不是单一的 `zh-CH.json`。

- [ ] **C-09** 检查 `messages/zh-CN/` 和 `messages/en-US/` 下的 namespace 文件是否一一对应，无缺失。
...
- [ ] **I-01** 访问 `/`，确认正确重定向到 `/{defaultLocale}/`（即 `/zh-CN/`）。

- [ ] **I-02** 访问 `/en-US/`，确认页面正常加载，文案为英文。

- [ ] **I-03** 切换语言，确认 URL 前缀变化，文案实时切换，无全量刷新。

- [ ] **I-04** 未登录访问受保护路由（如 `/zh-CN/dashboard`），确认被重定向到登录页。
...
- [ ] **I-09** 已登录状态访问 `/zh-CN/login`，确认被重定向到首页或 dashboard。
