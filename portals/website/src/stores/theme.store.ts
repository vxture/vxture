...
export const useThemeStore = create<ThemeState>()(
  persist(
    (set, get) => ({
      ...
      availableThemes: [...THEME_CONSTANTS.AVAILABLE_THEMES] as unknown as ThemeConfig[],
      /** 是否为暗色模式 */
      isDarkMode: (THEME_CONSTANTS.DEFAULT_THEME as string) === (THEME_CONSTANTS.DARK_CLASS as string),
      /**
       * 设置主题（支持任意已注册主题）
       * @param theme 主题名
       */
      setTheme: (theme: string) => {
        ...
        if (currentTheme !== next) {
          ...
          set({
            theme: next,
            isDarkMode: next === (THEME_CONSTANTS.DARK_CLASS as string),
          });
        }
      },
      /**
      ...
      toggleTheme: () => {
        set((state: ThemeState) => {
          const next =
            state.theme === (THEME_CONSTANTS.DARK_CLASS as string)
              ? (THEME_CONSTANTS.DEFAULT_THEME as string)
              : (THEME_CONSTANTS.DARK_CLASS as string);
