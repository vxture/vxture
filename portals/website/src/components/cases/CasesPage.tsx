  ...
  const filteredCases = useMemo(() => {
    return CASES_DATA.items.filter((item) => {
      ...
      const matchesCategory =
        ...
        tags.some((tag) =>
          tag === t(CASES_DATA.categories.find((c) => c.slug === selectedCategory)?.nameKey ?? '')
        );
    ...
    <section className='vx-section-odd min-h-screen'>
      <div className='container mx-auto px-4 py-16'>
        ...
        <div className='text-center mb-12'>
          <h1 className='font-display text-4xl font-bold text-vx-gray-900 mb-4'>{t(CASES_DATA.titleKey)}</h1>
          <p className='text-xl text-vx-gray-600 max-w-3xl mx-auto'>{t(CASES_DATA.subtitleKey)}</p>
        </div>
          ...
          {filteredCases.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className='p-6'>
                  <h3 className='text-xl font-semibold text-vx-gray-900 mb-2'>{title}</h3>
                  <p className='text-vx-gray-600 mb-4 line-clamp-3'>{description}</p>
                  ...
                  <div className='flex justify-between items-center'>
                    <span className='text-sm text-vx-gray-500'>{item.publishedAt}</span>
                    <Link
                      href={item.cta.href}
