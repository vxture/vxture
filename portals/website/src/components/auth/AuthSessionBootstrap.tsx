...
export function AuthSessionBootstrap() {
  ...
  useEffect(() => {
    const restoreIfStale = () => {
      const now = Date.now();
      if (inFlightRef.current || now - lastRestoreAtRef.current < SESSION_RESTORE_THROTTLE_MS) {
        return;
      }

      lastRestoreAtRef.current = now;
      inFlightRef.current = true;
      void useAuthStore.getState().restoreSession({ silent: true }).finally(() => {
    ...
    const intervalId = window.setInterval(restoreIfStale, SESSION_SYNC_INTERVAL_MS);
