  ...
  return (
    <ShellUserMenu
      user={{
        ...
        avatarFallback:
          Array.from(displayName.trim() || "V")[0]?.toLocaleUpperCase() ?? "V",
        badges: [
          { key: "role", label: roleLabel },
          ...(showPersonalBadge
            ? [{ key: "personal", label: t("badges.personal") }]
            : []),
          ...(showOrganizationBadge
            ? [{ key: "organization", label: t("badges.organization") }]
            : []),
        ],
      }}
      openLabel={t("open")}
...
export default function Header() {
  ...
  const handleLogout = async (
    ...
    try {
      ...
      addNotification(
        error instanceof Error ? error.message : t("userMenu.logoutFailed"),
        "error",
      );
    } finally {
  ...
  const handleSwitchUser = () =>
    handleLogout("/signin", t("userMenu.switchReady"));
