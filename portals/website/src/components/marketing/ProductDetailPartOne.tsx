      ...
      >
        <div className='mx-auto max-w-7xl px-6 lg:px-8 xl:max-w-screen-2xl'>
          <div className='flex flex-col gap-3 md:flex-row md:items-end md:justify-between'>
            <div>
              <p className='text-sm font-semibold text-vx-brand-600 dark:text-vx-brand-300'>{t('capabilities.eyebrow')}</p>
              <h2 className='font-display mt-2 text-3xl font-bold text-vx-gray-900 dark:text-vx-white'>{t('capabilities.title')}</h2>
            </div>
          ...
          <div className='mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
            {capabilities.map((item, index) => (
              ...
              >
                ...
                <h3 className='text-base font-semibold text-vx-gray-900 dark:text-vx-white'>{item.title}</h3>
                <p className='mt-3 text-sm leading-6 text-vx-gray-600 dark:text-vx-gray-300'>{item.description}</p>
                <ul className='mt-5 space-y-2'>
                  {item.points.map((point) => (
                    <li key={point} className='flex gap-2 text-sm text-vx-gray-600 dark:text-vx-gray-300'>
                      <Icon name='check' className='mt-0.5 h-4 w-4 text-vx-brand-500' />
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
        ...
        <div className='vx-website-split-grid vx-website-split-grid--34 mx-auto grid max-w-7xl gap-10 px-6 lg:px-8 xl:max-w-screen-2xl'>
          ...
          <div className='grid gap-4 md:grid-cols-4'>
            {workflow.map((step, index) => (
              <div key={step.title} className='relative border-l border-vx-brand-200 pl-5 dark:border-vx-brand-900/70'>
                ...
                <h3 className='mt-2 text-base font-semibold text-vx-gray-900 dark:text-vx-white'>{step.title}</h3>
                <p className='mt-2 text-sm leading-6 text-vx-gray-600 dark:text-vx-gray-300'>{step.description}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className='grid gap-4 md:grid-cols-3'>
              {scenarios.map((scenario) => (
                ...
                >
                  ...
                  <p className='mt-3 text-sm leading-6 text-vx-gray-600 dark:text-vx-gray-300'>{scenario.description}</p>
                </article>
              ))}
            </div>
          </div>
