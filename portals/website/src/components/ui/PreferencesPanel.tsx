...
export default function PreferencesPanel({
  ...
  const renderOptionGroup = <TValue extends string>(
    ...
    <div className='space-y-2'>
      <Label className='text-xs font-semibold text-vx-gray-500 dark:text-vx-gray-400 uppercase tracking-wider'>
        {title}
      </Label>
          ...
          <button
            ...
            className={`
              ...
              ${selectedValue === option.value
                ? 'border-vx-cyan-600 bg-vx-cyan-50 dark:bg-vx-brand-950/30 text-vx-cyan-700 dark:text-vx-info-300'
                : 'border-vx-gray-200 dark:border-vx-gray-700 hover:border-vx-gray-300 dark:hover:border-vx-gray-600 text-vx-gray-700 dark:text-vx-gray-300'
              }
            `}
          >
    ...
    <div className='space-y-2'>
      ...
      <div className='relative'>
        <select
          ...
          className='w-full px-3 py-2 rounded-lg border border-vx-gray-200 dark:border-vx-gray-700 bg-vx-surface dark:bg-vx-gray-800 text-vx-gray-900 dark:text-vx-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-vx-cyan-600'
        >
        ...
        <Icon name='chevron-down' className='absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-vx-gray-400 pointer-events-none' />
      </div>
    </div>
  );
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align='end'
        sideOffset={8}
        className='w-80 p-4 bg-vx-surface dark:bg-vx-gray-800 border-vx-gray-200 dark:border-vx-gray-700'
      >
        {/* 标题区 */}
        <div className='flex items-center justify-between mb-4'>
          <div className='flex items-center gap-2'>
            <Icon name='settings' className='w-5 h-5 text-vx-gray-600 dark:text-vx-gray-400' />
            <h3 className='font-semibold text-vx-gray-900 dark:text-vx-gray-100'>
              {t('title')}
            </h3>
          </div>
          {isLoggedIn ? (
            <span className='px-2 py-0.5 bg-vx-success-100 dark:bg-vx-success-900/30 text-vx-success-700 dark:text-vx-success-400 text-xs font-medium rounded-full'>
              {t('status.loggedIn')}
            </span>
          ) : (
            <span className='px-2 py-0.5 bg-vx-gray-100 dark:bg-vx-gray-700 text-vx-gray-600 dark:text-vx-gray-400 text-xs font-medium rounded-full'>
              {t('status.guest')}
            </span>
          )}
        </div>
        ...
        <div className='space-y-4'>
          ...
          {/* 分隔线 */}
          <div className='h-px bg-vx-gray-200 dark:bg-vx-gray-700' />
        ...
        {/* 底部提示 */}
        <div className='mt-4 pt-3 border-t border-vx-gray-200 dark:border-vx-gray-700'>
