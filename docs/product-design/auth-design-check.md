...
| 维度 | 运营账号 | 租户账号 |
|------|----------|----------|
| 使用产品 | admin.vxture.com | tenant.vxture.com · ruyin.ai · agent |
...
tenant.vxture.com   →  tenant portal   →  tenant-bff   租户账号专用
...
```
.vxture.com   →  admin · tenant · auth 共享同一 Cookie
...
```
① 用户在 tenant.vxture.com 已登录，触发跳转 ruyin.ai 的操作
...
tenant-bff   →  userType 必须为 tenant_user，否则返回 403
ruyin-bff    →  userType 必须为 tenant_user，否则返回 403
```

### 租户数据隔离原则

tenant-bff 和 ruyin-bff 的所有业务路由，tenantId 只能从 JWT 上下文中获取，所有数据查询必须携带此 tenantId 作为过滤条件，禁止从请求参数接收 tenantId。
...
| tenant-bff | NestJS | 仅接受 tenant_user JWT |
...
- [ ] tenant-bff 和 ruyin-bff 所有路由必须验证 `userType === "tenant_user"`
