...
4. P1：继续收敛 Admin 管理域。状态：已完成机械拆分；`admin-shell.css` 已拆成 core/sidebar/nav/responsive/content/locale/compact 模块；`admin-shell-core.css` 已按 layout/brand/actions/user 拆分；`admin-shell-nav.css` 已按 scroll/items/footer/collapsed 拆分；`admin-assistant.css` 已拆成 panel/conversation/messages/composer/responsive 模块；`admin-permissions.css` 已拆成 core/tree/tree-node/dialog 模块；`admin-permissions-tree-node.css` 已按 title/detail/status/structure 拆分；`admin-service-health.css` 已拆成 core/summary/catalog/responsive 模块；`admin-operations.css` 已拆成 controls/audit/announcements/skills/dialog/tickets 模块；`admin-directory.css` 已拆成 layout/list/status/cards/actions/pagination 模块；`admin-platform-autonomy.css` 已拆成 metrics/domains/panels/risks/responsive 模块；`admin-products.css` 已拆成 summary-toolbar/plans/releases/pricing/responsive 模块；`admin-governance.css` 已拆成 list/status/cards/responsive 模块；`admin-auth-captcha.css` 已拆成 modal/surface/puzzle/states 模块；`admin-roles.css` 已拆成 cards/permission-dialog/auth-dialog 模块；`admin-management.css` 保持稳定聚合入口，具体规则分布到 core/directory/models/commerce/products/tenant-workspace 模块；`admin-overview.css` 已拆成 core/metrics/business/products/models/service/responsive 模块；`admin-overview-core.css` 已按 cards/header/section/period/tones 拆分；`admin-overview-service.css` 已按 blocks/metrics/data/tips 拆分；`admin-overview-models.css` 已按 metrics/categories/rows 拆分；`admin-management-commerce.css`、`admin-management-directory.css`、`admin-tenant-detail.css` 已按交易、目录、租户详情域拆分；`admin-management-directory-commerce.css` 已按订阅、网格、解决方案、交易和增长域拆分；`admin-management-commerce-transactions.css` 已按 dialogs/orders/billing/invoices/payments 拆分；`admin-management-commerce-subscriptions.css` 已按 list/detail/actions/dialog 拆分；`admin-management-products-capability.css` 已按 shared/summary/detail/lists 拆分；`admin-tenant-detail-shell.css` 已按 summary/header/collapsed/actions/metrics 拆分；`admin-tenant-detail-members.css` 已按 table/toolbar/list/cards 拆分；`admin-tenant-detail-config.css` 已按 blocks/fields/review/kv/notes 拆分；`admin-management-models-rows.css` 已按 gateway/name/role-directory/actions/pills/shared 拆分；`admin-management-pills-products.css` 已按 roles/product-types/plans/solutions/service-plans 拆分；`admin-management-pills-commerce.css` 已按 subscriptions/orders/billing/invoices/payments 拆分；`admin-management-models.css`、`admin-management-pills.css`、`admin-management-products.css` 已按模型、状态标识、产品服务域拆分。验收：admin build、`pnpm lint:design` 通过；大入口保持 import-only。
5. P1：补强分层 guardrail。状态：已完成；`ds/no-style-entry-rules` 已约束 DS/platform、DS/console、admin/console/website/ruyin/Vela globals、admin shell、admin shell core、admin shell nav、admin assistant、admin auth captcha、admin directory、admin governance、admin permissions、admin permissions tree node、admin platform autonomy、admin products、admin roles、admin service health、admin operations、admin management 系列、admin management commerce subscriptions、admin management commerce transactions、admin management directory commerce、admin management models rows、admin management pills commerce、admin management pills products、admin management products capability、admin overview、admin overview core、admin overview models、admin overview service、tenant detail、tenant detail config、tenant detail shell 和 tenant detail members 大入口保持 import-only，`platform-*` 模块继续纳入 DS semantic CSS 约束。验收：`pnpm lint:design` 通过；baseline 仍为空。
