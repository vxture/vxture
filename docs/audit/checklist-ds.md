...
5. P1：补强分层 guardrail。状态：已完成；`ds/no-style-entry-rules` 已约束 DS/platform、DS/console、admin/console/website/ruyin/Vela globals、admin shell、admin assistant、admin auth captcha、admin directory、admin governance、admin permissions、admin platform autonomy、admin products、admin roles、admin service health、admin operations、admin management 系列、admin management commerce subscriptions、admin management commerce transactions、admin management directory commerce、admin management models rows、admin management products capability、admin overview、admin overview core、admin overview models、admin overview service、tenant detail、tenant detail shell 和 tenant detail members 大入口保持 import-only，`platform-*` 模块继续纳入 DS semantic CSS 约束。验收：`pnpm lint:design` 通过；baseline 仍为空。
