...
| 类型 | 代表 BFF | 核心职责 |
|------|---------|---------|
| **业务型 BFF** | auth / console / admin / vela / ruyin / agent-template | JWT 验证、tenant 解析、RBAC 守卫、请求聚合 |
...
**适合场景**：业务型 BFF。`@UseGuards(AuthGuard, TenantGuard)` 与中间件链（Auth → Tenant → Permission → Router）天然对应；DI 使测试中可替换 Redis/HTTP 依赖；Guards 可跨 BFF 复用。
...
- **业务型 BFF → NestJS**（默认）：Guard/DI/Module 结构统一，便于跨 BFF 复用中间件
