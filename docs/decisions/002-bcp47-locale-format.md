...
平台初期 i18n 使用 `zh` / `en` 简写格式作为 locale 标识。上线后发现以下问题：
...
**缺点**：URL 从 `/zh/` 改为 `/zh-CN/`，已存在的书签链接会 404（需 301 重定向）。
...
URL 301 重定向通过 Next.js middleware 实现（`/zh/` → `/zh-CN/`），对 SEO 友好，不造成死链。
...
**负面：**
- URL 路径段较之前更长（`/zh-CN/` vs `/zh/`）
- 需要维护从旧路径的 301 重定向（已实现，在 website middleware）
- 所有翻译文件目录从 `messages/zh/` 重命名为 `messages/zh-CN/`（一次性迁移，已完成）
