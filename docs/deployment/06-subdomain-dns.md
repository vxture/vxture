...
| Tunnel | ruyin                               | vxture-worker-H01  | Proxied  | Auto | Cloudflare Tunnel 内网穿透，直连 worker-02 |
...
3. `ruyin` 为 Cloudflare Tunnel 接入（Tunnel 名：`vxture-worker-H01`），绕过 ICP 备案限制直连 worker-02:3111
