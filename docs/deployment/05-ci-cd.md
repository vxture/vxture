...
| 文件                   | 触发条件                           | 运行环境      | 用途                                         |
| ---------------------- | ---------------------------------- | ------------- | -------------------------------------------- |
| `ci.yml`               | PR / push to main · beta · develop | ubuntu-latest | Quality Gate、构建、测试与安全审计           |
...
CI 内部包含 Format、Audit、Quality Gate、Build、Test · Coverage、SonarQube 等 job。`docker.yml` 独立触发（不依赖 CI），`deploy-worker-01.yml` 在 `docker.yml` 完成后自动触发。
...
当 `beta` 被 fast-forward 到 `develop`，会触发 beta 镜像构建；当 `main` 被 fast-forward 到 `beta`，会触发正式镜像构建和 worker-01 自动部署。promotion 操作必须记录源 commit、目标 commit、操作者和时间，确保部署追溯链不依赖 merge commit。
