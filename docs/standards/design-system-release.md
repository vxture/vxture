...
版本：1.0.0
日期：2026-05-30
范围：`@vxture/design-system`、`@vxture/shared`、`Publish Design System` workflow

本文定义 DS 包的版本判断、发布准备、dry run、真实发布和发布后验证流程。DS 包发布必须走 PR、CI、merge、workflow 验证流程，禁止从本地直接 `pnpm publish` 到 GitHub Packages。
...
| 入口     | 触发方式                             | 使用场景              |
| -------- | ------------------------------------ | --------------------- |
| 手动发布 | `workflow_dispatch`，`dry_run=false` | 常规 DS 包发布        |
...
- `.github/workflows/design-system-publish.yml`
