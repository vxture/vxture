...
```text
https://auth.vxture.com/auth/oauth/dingtalk/callback
```

如果生产主域不是 `www.vxture.com`，使用实际 website 域名：

```text
https://你的域名/auth/oauth/dingtalk/callback
```

本地走 gateway 时：

```text
http://localhost:3090/auth/oauth/dingtalk/callback
```
...
```text
GET /auth/oauth/dingtalk/start
GET /auth/oauth/dingtalk/callback
```
