...
JWT `userType = tenant_user`，`authScope = tenant_console`。Vela 智能助手嵌入 Console（`ConsoleVelaPanel.tsx`）。
...
| `/model-gateway`   | 模型网关配置（租户级模型访问）   | capabilities   | ✅ 已完成 |
...
权限模型详见 [`docs/design/permissions.md`](../../../design/permissions.md)。

---

## Vela 接入点

- 入口组件：`app/[locale]/(console)/ConsoleVelaPanel.tsx`
- Surface：`console`，userType：`tenant_user`
- 状态：⚠️ UI 占位已存在，待 Vela 三端接通（T06）
