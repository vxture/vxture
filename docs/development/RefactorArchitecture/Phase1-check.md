...
### 文件命名
- ✅ 有泛名文件（authConfig.ts、i18nConfig.ts、themeConfig.ts）→ 应该是 *.constants.ts
- ✅ 类型文件以 *.types.ts 结尾
- ✅ 工具文件不是 *.utils.ts 结尾（debug.ts、scroll.ts）→ 应该是 *.utils.ts
...
1. **高优先级**：
   ...
   - ✅ @vxture/shared 文件命名不符合规范（*.constants.ts、*.utils.ts）
...
│   ├── i18nConfig.ts → i18n.constants.ts
