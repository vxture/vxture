...
```bash
pnpm -F @vxture/bff-auth dev          # port 3090
pnpm -F @vxture/bff-vela dev          # port 3121
pnpm -F vela-server dev               # port 3122
pnpm -F @vxture/bff-admin dev         # port 3031（Vela 宿主）
pnpm -F @vxture/admin dev             # port 3030
```
...
# JWT（本地随机值即可，不要与生产共用）
JWT_ACCESS_SECRET=local-dev-access-secret-at-least-64-chars-long-xxxxxxxxxx
...
COOKIE_DOMAIN_PLATFORM=localhost
COOKIE_DOMAIN_RUYIN=localhost
```
