...
| vela-bff    | 3041 | 透传宿主 Cookie                               | portals/admin + portals/console |
| ruyin-bff   | 3051 | `vx_ruyin_session`                            | business/ruyin                  |
...
| `platform.model.manage`   | AI Gateway 模型管理                      |
...
**禁止**：admin-bff 不得查询 `ai_gateway.*`（旧表，已过渡至 `model.*`）。
...
const signResponse = await fetch(`${AUTH_BFF_URL}/auth/internal/sign`, {
  ...
  body: JSON.stringify({
    ...
    email: user.email,
    role: user.roleCode,
    source: "admin", // 'admin' | 'console' | 'ruyin'
  }),
});
```
...
Vela 使用独立 Prisma Client，公共 schema（`public`）下的 3 张表：

| 表             | 用途                                                  |
| -------------- | ----------------------------------------------------- |
| `VelaSession`  | 对话会话（userId + surface）                          |
| `VelaMessage`  | 消息记录（含 `toolCallId` 字段）                      |
| `VelaAuditLog` | 工具调用审计（三态：pending / confirmed / cancelled） |
...
Vela BFF 不直接查询 `platform_main` 数据库，通过 vela-server 内部 API 访问。
...
├── ai_gateway— ai_model, ai_provider, ai_model_grant, ai_model_cost_rate     ← 过渡期旧表
└── public    — VelaSession, VelaMessage, VelaAuditLog                        ← Vela 私有表
```
...
| `services/ai/gateway/prisma/schema.prisma`    | commerce（3张用量表）+ model（只读代理）                                    |
| `agent-server/vela/prisma/schema.prisma`      | public（Vela 3张表）                                                        |
...
| `AI_GATEWAY_URL`            | AI Gateway 地址（admin-bff ai-gateway 路由透传用） |
...
| 查询 `ai_gateway.*` 表                         | 旧表，admin-bff 应查 `model.*` 表                           |
