...
平台运营后台 BFF，覆盖租户管理、账单、订阅、产品、AI Gateway、运营人员权限等域。
middleware 顺序：`auth → capabilities → router`

**不签发 JWT**。登录流程：IP+账号限速 → 服务端滑块验证 → DB 密码校验 → 委托 `auth-bff POST /auth/internal/sign` 签发 Cookie。
...
**POST `/api/auth/captcha/challenge`** — 获取滑块验证码挑战（无鉴权）
...
**POST `/api/auth/login`** — 密码 + 滑块验证码登录（无鉴权）
...
### `/api/ai-gateway` — AI Gateway 模型管理

**需要能力：`platform.model.manage`**

> 所有接口透传到 AI Gateway HTTP API（`AI_GATEWAY_URL`），不直接操作数据库。

**GET `/api/ai-gateway/models?includeInactive=true`** — 模型列表

**POST `/api/ai-gateway/models`** — 创建模型

**PUT `/api/ai-gateway/models/:modelId`** — 更新模型

**POST `/api/ai-gateway/models/:modelId/activate`** — 激活模型

**POST `/api/ai-gateway/models/:modelId/deactivate`** — 停用模型

**DELETE `/api/ai-gateway/models/:modelId`** — 删除模型

**GET `/api/ai-gateway/grants?tenantId=&modelId=`** — 模型授权列表

**POST `/api/ai-gateway/grants`** — 创建授权

**PUT `/api/ai-gateway/grants/:grantId`** — 更新授权

**POST `/api/ai-gateway/grants/:grantId/activate`** — 激活授权

**DELETE `/api/ai-gateway/grants/:grantId`** — 停用/删除授权
