...
- **集中化密钥管理**：`JWT_SECRET` 只出现在 auth-bff 的 env 中，其他 BFF 只验证（verify），不签发（sign）
...
- **JWT_SECRET 唯一**：仅 auth-bff 拥有签发权，其他 BFF 只 verify
...
```bash
# JWT
JWT_SECRET=                     # ≥ 32 字符，所有 BFF 共用同一值
JWT_ACCESS_EXPIRES_IN=8h
JWT_REFRESH_EXPIRES_IN=30d

# Cookie Domain
COOKIE_DOMAIN_PLATFORM=.vxture.com
COOKIE_DOMAIN_RUYIN=.ruyin.ai   # 可选，ruyin OAuth 跨域
...
# 内部通信
AUTH_INTERNAL_TOKEN=            # 与 admin-bff 共享，用于 internal/sign 接口校验

# Portal 基址（forgot-password 邮件中重置链接用）
WEBSITE_BASE_URL=https://vxture.com
CONSOLE_BASE_URL=https://console.vxture.com

# Cloudflare Turnstile（可选，未配置时跳过验证）
