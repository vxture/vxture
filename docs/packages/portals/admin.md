...
Vela 智能助手嵌入点（admin surface），入口文件 `src/layout/VelaAdminChat.tsx`。
...
├── login/                      ← 登录页（无 layout）
├── (admin)/
│   ├── layout.tsx              ← AdminShell（含 Vela 入口）
│   ├── accounts/               ← 账号管理
│   ├── admin-roles/            ← 平台管理员角色
...
- 入口：`src/layout/VelaAdminChat.tsx`
- Surface：`admin`，userType：`platform_operator`
- 接通状态：✅ 一期完成（`bff/vela-bff` + `agent-server/vela` + `agent-studio/vela` 三端运行中）
