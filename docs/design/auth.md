...
```
https://vxture.com/auth-api/auth/oauth/dingtalk/callback
https://vxture.com/auth-api/auth/oauth/feishu/callback
https://vxture.com/auth-api/auth/oauth/wecom/callback
```
...
**环境变量**（配置于 `bff/auth-bff/.env.example`）：
...
```
生产：https://vxture.com/auth-api/auth/oauth/dingtalk/callback
...
- 前端只持有 site key，通过 `NEXT_PUBLIC_CF_TURNSTILE_TENANT_SITE_KEY` 或 `NEXT_PUBLIC_CF_TURNSTILE_ADMIN_SITE_KEY` 注入。
- `AuthTurnstile` 默认使用无感/交互优先模式，不能浮动遮挡用户协议、隐私政策等勾选项。
...
vela-bff     →  根据 x-vela-surface 分别要求 operator/platform-admin 或 tenant_user/tenant-console
```
...
| vela-server  | NestJS  | Vela AI 助手后端（嵌入 admin / console）    |
| Nginx        | —       | 反向代理，路由分发                          |
...
AUTH_INTERNAL_TOKEN=         # 内部 BFF 调用 /auth/internal/sign 的共享密钥，生产环境必须显式配置
...
CF_TURNSTILE_TENANT_ALLOWED_HOSTNAMES=localhost,127.0.0.1,vxture.com,ruyin.ai
NEXT_PUBLIC_CF_TURNSTILE_ADMIN_SITE_KEY=
CF_TURNSTILE_ADMIN_SECRET_KEY=
CF_TURNSTILE_ADMIN_ALLOWED_HOSTNAMES=localhost,127.0.0.1,admin.vxture.com
```
...
- Cloudflare Turnstile Managed 模式接入（tenant 端认证请求）
...
- 企业微信个人账号登录
- 多租户切换器 UI
- ruyin 前端完整登录页迁移到统一 DS 模板
- admin Turnstile 独立 surface 接入
