...
每个业务部署在独立的 Docker 网络，实现故障隔离：

- 平台控制面：`vx-platform` 网络（worker-01）
- 每个业务数据面：独立网络，如 `vx-vela-prod`、`vx-ruyin-prod`（worker-02）
...
业务服务（如 ruyin）通过 Cloudflare Tunnel 直达 worker-02，无需经过 worker-01 Nginx。

---

## 8.3 容器边界原则

平台容器（worker-01，`vx-platform` 网络）：

```txt
vx-nginx, vx-website, vx-admin, vx-console
vx-auth-bff, vx-website-bff, vx-admin-bff, vx-console-bff, vx-gateway-bff
vx-platform-pg, vx-platform-redis
```

业务容器（worker-02，各自独立网络）：

```txt
vx-vela-bff, vx-vela-server, vx-vela-pg, vx-vela-redis
vx-ruyin-bff, vx-ruyin-server, vx-ruyin-pg, vx-ruyin-redis
vx-ai-gateway, vx-ai-gateway-pg
```
