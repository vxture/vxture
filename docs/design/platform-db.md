...
|-------------------|-------------|---------|
| `account` | `identity` | 重命名；3 张新表；account.status 类型变更；password_hash 提出；account_profile 补 metadata |
| `tenancy` | `tenant` | 重命名；RBAC 3 张表迁出到 iam；tenant_config 改名；新增 invitation |
| ——（不存在） | `iam` | 全新；从 tenancy 迁入 3 张；新建 2 张 |
| `product` | `product` | ✅ 字段顺序不变 |
| `commerce` | `commerce` | ✅ 补 3 张缺失表 |
| `ai_gateway`（部分） | `model` | 4 张表迁入并改名；移除 1 字段；新建 model_policy |
| `platform` | `ops` | 重命名；5 张表去 `platform_` 前缀；新建 3 张 |
| `support` | `support` | ✅ 补 2 张缺失表 |
...
| status | VARCHAR(32) NOT NULL | `active` / `suspended` / `deleted`；原为 BOOLEAN，扩展为枚举以支持 suspended |
...
**`account_credential`** ✏️ — 登录凭证（从 account.password_hash 提出）
...
> 独立表的价值：可对 credential 表单独设 Row-Level Security，主账号查询路径不触碰密码哈希。

---

**`account_profile`** ✏️ — 账号扩展资料（补 metadata 扩展字段）
...
**`oauth_state`** ✏️ — OAuth 授权请求临时状态（原 `account_oauth_state`，append-only）
...
| revoke_reason | VARCHAR(64) NOT NULL | `logout` / `password_change` / `admin_force` |
...
| purpose | VARCHAR(32) NOT NULL | `register` / `login` / `bind` / `reset_password` |
...
**`login_attempt`** 🆕 — 登录尝试记录（限速用，append-only）
...
| result | VARCHAR(32) NOT NULL | `success` / `wrong_password` / `not_found` / `locked` |
...
|------|---------|------|
| `tenant` | `tenant` | ✅ |
| `tenant_member` | `tenant_member` | ✏️ 移除 role_id |
...
**`tenant_member`** ✏️ — 租户成员关系（移除 role_id 直接引用）
...
| `permission` | `tenancy.tenant_permission` | 📦 迁入，去除 scope 混合字段 |
...
**`permission`** 📦 — 权限项目录（原 `tenancy.tenant_permission`，去除 scope 混合设计）
...
✅ **现有 7 张表结构不变**：`plan`、`feature`、`plan_feature`、`plan_price`、`agent`、`agent_feature`、`plan_agent`。
...
**`tenant_credit`** 🆕 — 账户余额与赠送额度

| 字段 | 类型 | 说明 |
|------|------|------|
| id | UUID NOT NULL | PK |
| tenant_id | UUID NOT NULL | FK → tenant_subscription；全局唯一，每租户一行 |
...
| updated_at | TIMESTAMPTZ NOT NULL | 无 created_at / deleted_at，余额行永久存在 |

> 余额变动的审计流水在 `tenant_transaction` 账本，此表只存当前快照。
...
|------|---------|------------|------|
| `provider` | `ai_provider` | `ai_gateway` | 📦 迁入，表名简化 |
| `model` | `ai_model` | `ai_gateway` | 📦 迁入，表名简化，移除 1 字段 |
...
**`model`** 📦 — 模型定义（原 `ai_model`，移除 `api_key_env_var`）
...
| 表名 | 当前表名 | 状态 |
|------|---------|------|
| `admin` | `platform_admin` | ✏️ 表名去前缀 |
| `role` | `platform_role` | ✏️ 表名去前缀 |
| `permission` | `platform_permission` | ✏️ 表名去前缀 |
| `role_permission` | `platform_role_permission` | ✏️ 表名去前缀 |
| `setting` | `platform_config` | ✏️ 表名去前缀并语义化 |
...
**`admin`** ✏️ — 运营人员账号（原 `platform_admin`，仅去前缀）
...
| status_code | VARCHAR(32) NOT NULL DEFAULT 'active' | `active` / `suspended` / `locked` |
| sort | INT NOT NULL DEFAULT 999 | |
...
**`role`** ✏️ — 运营角色（原 `platform_role`，仅去前缀）
...
**`permission`** ✏️ — 运营权限项（原 `platform_permission`，仅去前缀）
...
**`role_permission`** ✏️ — 运营角色权限关联（原 `platform_role_permission`，复合 PK）
...
**`setting`** ✏️ — 全局平台配置（原 `platform_config`）
...
| meta | JSONB NULL | 额外触达条件或展示配置（如自定义 CTA、链接、图标等） |
| created_by | UUID NOT NULL | |
| created_at | TIMESTAMPTZ NOT NULL | |
| updated_at | TIMESTAMPTZ NOT NULL | |
| deleted_at | TIMESTAMPTZ NULL | |
...
|------|---------|------|
| `ticket` | `ticket` | ✅ |
| `ticket_event` | `ticket_event` | ✅ append-only |
...
| source | VARCHAR(64) NOT NULL DEFAULT 'admin' | `console` / `admin` / `api` |
...
**`ticket_event`** ✅ — 工单事件流水（append-only）
...
| status | VARCHAR(32) NOT NULL | `sent` / `failed` / `bounced` |
...
```sql
-- identity.account
ALTER TABLE identity.account ALTER COLUMN status TYPE VARCHAR(32) USING CASE WHEN status THEN 'active' ELSE 'deleted' END;
ALTER TABLE identity.account DROP COLUMN password_hash;
CREATE TABLE identity.account_credential ( account_id UUID PRIMARY KEY REFERENCES identity.account(id), ... );
INSERT INTO identity.account_credential SELECT id, password_hash FROM ... ;

-- tenant.tenant_member
ALTER TABLE tenant.tenant_member DROP COLUMN role_id;

-- tenant.tenant_config → tenant_setting（Step 2 已含重命名）
ALTER TABLE tenant.tenant_config RENAME TO tenant_setting;

-- model.model
ALTER TABLE model.model DROP COLUMN api_key_env_var;
