...
所有 Commerce 数据存储于 `vx-platform-pg` 的两个 Schema：
...
**关键约束**：试用期内配额来自 `plan_feature.quota_value`，覆盖到 `tenant_subscription_quota`，ai-gateway 通过查 `tenant_subscription_quota` 校验配额，无需访问 product 层。
...
LLM Provider 返回结果
  │
  ▼
ai-gateway 写入 commerce.tenant_usage_event（Append-only）
  input_quota, output_quota, used_quota（CHECK: used = input + output）
  model_code, agent_id, feature_code, tenant_id, request_id
  │
  ▼
异步聚合 Job（定时，非阻塞主路径）
  读取新的 usage_event
...
> **重要**：实时配额检查读 `tenant_usage_summary.summary`，不遍历 `usage_event`，保证低延迟。
...
```
调用方传入 (tenantId, agentId, featureCode)
  │
  ▼
① 检查 tenant_subscription_override
     是否有针对该 (tenantId, agentId, featureCode) 的定制配额
...
`commerce.tenant_transaction` 有数据库规则阻止 `UPDATE` 和 `DELETE`：

```sql
-- 任何对 tenant_transaction 的修改操作都会触发异常
-- 需要更正时只能追加反向冲正记录
```
...
- `used_quota = input_quota + output_quota`，由 CHECK 约束强制执行：
  ```sql
  CONSTRAINT check_used_quota CHECK (used_quota = input_quota + output_quota)
  ```
