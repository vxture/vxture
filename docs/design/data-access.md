...
> 版本：1.0.0 | 2026-05-15
> 上级文档：[`docs/design/database.md`](database.md)（数据库顶层架构）
...
│  agent-studio/vela  agent-studio/ruyin                       │
│                                                              │
│  ► 只通过 HTTPS 与 BFF 通信                                   │
│  ► 永远看不到数据库                                           │
└────────────────────────────┬─────────────────────────────────┘
...
| `model` | model-service | admin-bff, ai-gateway |
...
- AI Gateway DB 使用独立角色 `aigateway_svc`，不使用上述角色
...
| 规则 | auth-bff | website-bff | console-bff | admin-bff | vela-bff | ruyin-bff |
|------|---------|------------|------------|----------|---------|---------|
| 禁止直连 DB（写） | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 禁止直连 DB（读） | ✅ | ✅ | ✅ | ⚠️ reporting_ro 只读例外 | ✅ | ✅ |
| 须持有 DATABASE_URL | ❌ | ❌ | ❌ | ⚠️ reporting_ro 只读 | ❌ | ❌ |
| 数据来源 | identity-service, tenant-service | identity-service, tenant-service | identity-service, tenant-service, commerce-service | 各 service API + reporting_ro | vela-server | ruyin-server |
...
| 数据库 | 访问模式 | 说明 |
|--------|---------|------|
| `aigateway_main` | ai-gateway 服务自管 Prisma | 独立实例，不经 platform service 层 |
| `vela_main` / `ruyin_main` | agent-server 自管 Prisma | 业务 DB，通过 BFF 与平台通信 |

ai-gateway 读取 `platform_main.model` schema 数据时，须调用 model-service HTTP API，不得持有 platform DB 连接。
