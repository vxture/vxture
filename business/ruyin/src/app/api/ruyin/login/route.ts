...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const token = typeof tokenPayload === 'object' && tokenPayload !== null
  ...
  try {
    ...
    return NextResponse.json({ status: 'ruyin_bff_unavailable' }, { status: 503 });
  }
  ...
  return jsonWithUpstreamCookies({ status: 'authenticated' }, { status: 200 }, callbackResponse);
}
