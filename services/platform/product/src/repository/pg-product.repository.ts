...
export class PgProductRepository {
  ...
  async listAgents(
    ...
    if (params.keyword) {
      conditions.push(`(agent_code ilike $${idx} or agent_name ilike $${idx})`);
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from product.agent where ${where}`,
        values,
      ),
  ...
  async getAgentById(id: string): Promise<AgentRecord | null> {
    const result = await this.pool.query<AgentRow>(
      `select * from product.agent where id = $1 and deleted_at is null limit 1`,
      [id],
    );
  ...
  async listFeatures(params: ListFeaturesParams): Promise<FeatureRecord[]> {
    ...
    if (params.featureType) {
      conditions.push(`feature_type = $${idx++}`);
      values.push(params.featureType);
    }
    if (params.parentCode) {
      conditions.push(`parent_code = $${idx++}`);
      values.push(params.parentCode);
    }
  ...
  async getFeatureByCode(featureCode: string): Promise<FeatureRecord | null> {
    const result = await this.pool.query<FeatureRow>(
      `select * from product.feature where feature_code = $1 and deleted_at is null limit 1`,
      [featureCode],
    );
    const row = result.rows[0];
  ...
  async listPlans(
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from product.plan where ${where}`,
        values,
      ),
  ...
  async getPlanDetail(id: string): Promise<PlanDetail | null> {
    const [planResult, pricesResult, featuresResult, agentsResult] =
      await Promise.all([
        this.pool.query<PlanRow>(
          `select * from product.plan where id = $1 and deleted_at is null limit 1`,
          [id],
        ),
        this.pool.query<PlanPriceRow>(
          `select * from product.plan_price where plan_id = $1 and deleted_at is null order by sort asc`,
          [id],
        ),
        this.pool.query<{
          feature_id: string;
         ...
         where ppf.plan_id = $1 and ppf.deleted_at is null`,
          [id],
        ),
        this.pool.query<{
          agent_id: string;
         ...
         where ppa.plan_id = $1 and ppa.deleted_at is null`,
          [id],
        ),
      ]);
  ...
  async setPlanFeatures(input: SetPlanFeaturesInput): Promise<void> {
    ...
    try {
      ...
      await client.query(
        `update product.plan_feature set deleted_at = now() where plan_id = $1 and deleted_at is null`,
        [input.planId],
      );
      for (const f of input.features) {
        await client.query(
          ...
          [
            ...
            f.quotaValue ?? 0,
            f.isUnlimited ?? false,
            f.configJson ? JSON.stringify(f.configJson) : null,
            input.operatorId,
          ],
        );
      }
