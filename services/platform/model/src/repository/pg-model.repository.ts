...
export class PgModelRepository {
  ...
  async getProviderById(id: string): Promise<ModelProviderRecord | null> {
    const result = await this.pool.query<ProviderRow>(
      `select * from model.provider where id = $1 and deleted_at is null limit 1`,
      [id],
    );
  ...
  async getProviderByCode(
    ...
    const result = await this.pool.query<ProviderRow>(
      `select * from model.provider where provider_code = $1 and deleted_at is null limit 1`,
      [providerCode],
    );
    const row = result.rows[0];
    return row ? this.mapProvider(row) : null;
  ...
  async listModels(
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from model.model where ${where}`,
        values,
      ),
  ...
  async getModelById(id: string): Promise<ModelDefinitionRecord | null> {
    const result = await this.pool.query<ModelRow>(
      `select * from model.model where id = $1 and deleted_at is null limit 1`,
      [id],
    );
    const row = result.rows[0];
  ...
  async getModelByCode(
    ...
    const result = await this.pool.query<ModelRow>(
      `select * from model.model where model_code = $1 and deleted_at is null limit 1`,
      [modelCode],
    );
    const row = result.rows[0];
    return row ? this.mapModel(row) : null;
