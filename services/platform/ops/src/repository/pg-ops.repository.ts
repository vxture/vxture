...
export class PgOpsRepository {
  ...
  async findAdminById(id: string): Promise<AdminRecord | null> {
    const result = await this.pool.query<AdminRow>(
      `select * from ops.admin where id = $1 and deleted_at is null limit 1`,
      [id],
    );
    const row = result.rows[0];
    return row ? this.mapAdmin(row) : null;
    ...
    const [countResult, rowsResult] = await Promise.all([
      ...
      this.pool.query<AdminRow>(
        `select a.*, r.name_en as role_name_en
         from ops.admin a left join ops.role r on r.id = a.role_id
         where ${where}
  ...
  async createAdmin(input: CreateAdminInput): Promise<AdminRecord> {
    const result = await this.pool.query<AdminRow>(
      `insert into ops.admin (
        role_id, username, email, phone, display_name, password_hash,
  ...
  async updateAdminPassword(id: string, passwordHash: string): Promise<void> {
    await this.pool.query(
      `update ops.admin set password_hash = $2, password_changed_at = now(), updated_at = now()
  ...
  async recordLogin(id: string, ip: string, success: boolean): Promise<void> {
    if (success) {
      await this.pool.query(
        `update ops.admin set last_login_at = now(), last_login_ip = $2,
          login_failure_count = 0, locked_until = null, updated_at = now()
         where id = $1`,
        [id, ip],
      );
    } else {
      await this.pool.query(
        `update ops.admin set login_failure_count = login_failure_count + 1, updated_at = now()
         where id = $1`,
        [id],
      );
    }
  ...
  async softDeleteAdmin(id: string, deletedBy?: string): Promise<void> {
    await this.pool.query(
      `update ops.admin set deleted_at = now(), updated_by = coalesce($2, updated_by), updated_at = now()
       where id = $1 and deleted_at is null`,
      [id, deletedBy ?? null],
    );
  }
  ...
  async listRoles(): Promise<RoleRecord[]> {
    const result = await this.pool.query<RoleRow>(
      `select * from ops.role order by sort asc, created_at asc`,
    );
  ...
  async createRole(input: CreateRoleInput): Promise<RoleRecord> {
    const result = await this.pool.query<RoleRow>(
      `insert into ops.role (
        role_code, name_en, name_i18n_key, description, description_i18n_key,
  ...
  async listPermissions(): Promise<PermissionRecord[]> {
    const result = await this.pool.query<PermRow>(
      `select * from ops.permission where is_active = true order by sort asc`,
    );
