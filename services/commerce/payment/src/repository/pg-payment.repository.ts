...
export class PgPaymentRepository {
  ...
  async listPayments(params: ListPaymentsParams): Promise<ListPaymentsResult> {
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from commerce.tenant_payment where ${where}`,
        values,
      ),
      this.pool.query<PaymentRow>(
        `select * from commerce.tenant_payment where ${where}
  ...
  async getPaymentById(id: string): Promise<PaymentRecord | null> {
    const result = await this.pool.query<PaymentRow>(
      `select * from commerce.tenant_payment where id = $1 limit 1`,
      [id],
    );
  ...
  async listRefunds(params: ListRefundsParams): Promise<ListRefundsResult> {
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from commerce.tenant_refund where ${where}`,
        values,
      ),
      this.pool.query<RefundRow>(
        `select * from commerce.tenant_refund where ${where}
         order by created_at desc limit $${idx} offset $${idx + 1}`,
        [...values, pageSize, offset],
      ),
    ]);
  ...
  async getRefundById(id: string): Promise<RefundRecord | null> {
    const result = await this.pool.query<RefundRow>(
      `select * from commerce.tenant_refund where id = $1 limit 1`,
      [id],
    );
    const row = result.rows[0];
  ...
  async appendTransaction(
    ...
    const result = await this.pool.query<TransactionRow>(
      `insert into commerce.tenant_transaction (
        ...
        related_no, remark, operator_id, client_ip, created_by, created_at
