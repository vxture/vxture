...
export class PgBillingRepository {
  ...
  async listInvoices(params: ListInvoicesParams): Promise<ListInvoicesResult> {
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from commerce.tenant_invoice i where ${where}`,
        values,
      ),
  ...
  async getInvoiceById(id: string): Promise<InvoiceRecord | null> {
    const result = await this.pool.query<InvoiceRow>(
      `select * from commerce.tenant_invoice where id = $1 and deleted_at is null limit 1`,
      [id],
    );
  ...
  async getInvoiceDetail(id: string): Promise<InvoiceDetail | null> {
    const [invoiceResult, itemsResult] = await Promise.all([
      ...
      this.pool.query<InvoiceItemRow>(
        `select * from commerce.tenant_invoice_item where bill_id = $1 and deleted_at is null order by created_at`,
        [id],
      ),
    ]);
    ...
    try {
      ...
      const invoiceResult = await client.query<InvoiceRow>(
        `insert into commerce.tenant_invoice (
          ...
          currency, bill_status, bill_type, created_by, created_at, updated_at
        ) values (
          $1, $2, $3, $4, $5, $6,
          $7, 0, $7, 0,
          $8, 'unpaid', $9, $10, now(), now()
        ) returning *`,
        [
          input.tenantId,
          billNo,
      ...
      for (const item of input.items) {
        const itemResult = await client.query<InvoiceItemRow>(
          `insert into commerce.tenant_invoice_item (
            bill_id, tenant_id, agent_id, feature_id, subscription_id,
            item_name, item_type, item_unit, quantity, unit_price, total_amount,
            usage_record_id, remark, created_at, updated_at
  ...
  async updateInvoiceStatus(
    ...
    const result = await this.pool.query<InvoiceRow>(
      `update commerce.tenant_invoice set
        ...
        operator_id   = coalesce($6, operator_id),
        operate_remark = coalesce($7, operate_remark),
  ...
  async getCreditByTenantId(tenantId: string): Promise<CreditRecord | null> {
    const result = await this.pool.query<CreditRow>(
      `select * from commerce.tenant_credit where tenant_id = $1 limit 1`,
      [tenantId],
    );
    const row = result.rows[0];
    if (!row) return null;
