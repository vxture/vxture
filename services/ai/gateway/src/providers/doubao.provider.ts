...
export class DoubaoProvider extends BaseProvider {
  ...
  async *chatStream(request: ProviderChatRequest): AsyncGenerator<StreamEvent> {
    const response = await fetch(resolveChatCompletionsEndpoint(request.endpointUrl), {
      method: 'POST',
      headers: {
...
function parseOpenAiToolCalls(toolCalls: OpenAiToolCall[] | undefined): ToolCall[] {
    ...
    while (true) {
      ...
      while (nl !== -1) {
        ...
        try {
          ...
          yield { type: 'error', code: 'PARSE_FAILED', message: `Invalid SSE chunk: ${dataPayload}` };
          continue;
        }
