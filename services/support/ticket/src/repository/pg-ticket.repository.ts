...
export class PgTicketRepository {
  ...
  async listTickets(params: ListTicketsParams): Promise<ListTicketsResult> {
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from support.ticket where ${where}`,
        values,
      ),
      this.pool.query<TicketRow>(
        `select * from support.ticket where ${where}
  ...
  async getById(id: string): Promise<TicketRecord | null> {
    const result = await this.pool.query<TicketRow>(
      `select * from support.ticket where id = $1 and deleted_at is null limit 1`,
      [id],
    );
  ...
  async getEvents(ticketId: string): Promise<TicketEventRecord[]> {
    const result = await this.pool.query<EventRow>(
      `select * from support.ticket_event where ticket_id = $1 order by created_at asc`,
      [ticketId],
    );
  ...
  async listAuditLogs(params: {
    ...
    const [countResult, rowsResult] = await Promise.all([
      this.pool.query<{ count: string }>(
        `select count(*) as count from support.audit_log where ${where}`,
        values,
      ),
      this.pool.query<AuditRow>(
        `select * from support.audit_log where ${where}
