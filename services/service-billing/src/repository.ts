...
import { InvoiceStatus, PaymentStatus, PaymentMethod, BillingCycle } from './types';
...
export class BillingRepository {
  ...
  async createInvoice(input: CreateInvoiceInput): Promise<Invoice> {
    ...
    const newInvoice: Invoice = {
      ...
      dueDate: input.dueDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 默认30天后
      lineItems,
      notes: input.notes,
      createdAt: new Date(),
      updatedAt: new Date()
