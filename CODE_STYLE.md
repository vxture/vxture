...
4. 必须添加必要注释，注释解释“为什么”而不是“是什么”，不写无用注释

5. 禁止使用未定义的类型、变量、路径

6. 遵循项目分层架构，禁止跨层调用

7. 早期返回：减少嵌套层级，提升代码可读性

8. 提取函数：复杂逻辑拆分提取，降低认知复杂度

9. 类型安全：充分利用 TypeScript 类型系统，杜绝无约束类型使用

10. 一致性：全项目保持统一的代码风格、命名规范和注释格式
...
2.  Props 解构：解构传入的 props、props 校验

3.  计算属性/工具函数：useMemo 计算属性、组件内部工具函数

4.  事件处理：回调函数（优先使用 useCallback 包裹）

5.  Effects：useEffect 代码（按业务逻辑优先级排列）

6.  返回：最后的 return 语句或渲染逻辑
...
- 所有icon除自定义icon外，均来自react-icons库的tabler-icons（导入路径：react/tb）

- 导入规范：按需导入，禁止一次性导入整个tabler-icons库，减少打包体积

  - 正确：import { TbUser, TbSettings } from 'react-icons/tb';

  - 错误：import * as Tb from 'react-icons/tb';

- 自定义icon：需统一存放于 shared/icons/ 目录，命名遵循kebab-case格式（例：custom-search.svg），且需与tabler-icons风格保持一致

#### 示例代码（可直接复用）

客户端组件中使用tabler-icons，结合TailwindCSS调整样式，符合项目规范：
...
// 常量定义
// ============================================================================
import { TbUser, TbSettings, TbSearch } from 'react-icons/tb';

// ============================================================================
// 组件实现
// ============================================================================
export const IconButton = ({ icon, onClick, className }: IconButtonProps) => {
  // 图标映射，避免switch-case冗余
  const iconMap = {
    user: <TbUser className="w-5 h-5" />,
    settings: <TbSettings className="w-5 h-5" />,
    search: <TbSearch className="w-5 h-5" />
  };

  return (
    <button
      onClick={flex items-center gap-2 px-3 py-1 rounded-md bg-gray-100 hover:bg-gray-200 ${className}`}
...
示例说明：按需导入图标，结合组件封装复用，样式统一使用TailwindCSS，符合项目所有编码规范。
...
**修复**: 组件 Props 接口所有属性添加 readonly 修饰符（详见 8. 接口和类型定义规范）
...
2.  复杂场景：使用 Zustand 状态管理，统一管理全局/模块级数据

3.  局部场景：组件组合、插槽（Slot）传递，减少 props 层级
...
- shared/          工具、常量、类型（含 constants 静态变量目录、icons 自定义图标目录）
