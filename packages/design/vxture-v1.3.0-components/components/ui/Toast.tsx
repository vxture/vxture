  ...
  return (
    <ToastContext.Provider value={{ toast, dismiss }}>
      ...
      <div className="vx-toast-viewport" role="region" aria-label="Notifications">
        {toasts.map((t) => (
          <div key={t.id} className={cn('vx-toast', `vx-toast--${t.tone}`)} role="alert">
            <span className="vx-toast__icon" aria-hidden>{TONE_ICON[t.tone]}</span>
            <div className="vx-toast__body">
              <div className="vx-toast__title">{t.title}</div>
              {t.description && <div className="vx-toast__desc">{t.description}</div>}
            </div>
            <button
              type="button"
