...
export default function Header() {
  ...
  // 错误状态或数据缺失
  if (error || !header) {
    console.error('Header data loading error:', error);
        ...
        <div className='max-w-7xl xl:max-w-screen-2xl 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='flex justify-between items-center h-16'>
            ...
            <div className='flex-shrink-0 flex items-center space-x-2'>
              <img
                src={fallbackHeader.logo.image}
                alt={fallbackHeader.logo.alt}
              ...
              >
                {fallbackHeader.logo.text}
              </h1>
            </div>

            {/* Fallback Navigation */}
            <nav className='hidden md:flex space-x-8'>
              {fallbackHeader.nav.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
            ...
            {/* Theme Switcher + Fallback Actions */}
            <div className='flex space-x-4 items-center'>
              ...
              {/* Language Switcher (Fallback) - 去掉背景框，加强图标对比 */}
              <div ref={languageMenuRef} className='relative'>
                ...
                >
                  ...
                  <FiGlobe className={`w-5 h-5 ${isScrolled ? 'text-gray-700' : 'text-cyan-400'}`} />
                </button>
              ...
              {/* Fallback CTA Buttons - 固定宽度 */}
              {fallbackHeader.actions.map((action, index) => (
                <a
                  key={action.label}
                  href={action.href}
                  className={
