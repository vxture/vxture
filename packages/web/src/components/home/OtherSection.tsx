...
export default function OtherSection() {
  ...
  const stickyRef = useRef<{ isSticky: boolean; placeholder: HTMLDivElement | null }>({
    isSticky: false,
    placeholder: null,
  });
  ...
  const handleScroll = () => {
    ...
    const headerHeight = 80; // 根据实际 header 高度调整
    const threshold = 180;   // 吸附触发距离
    const shouldSticky = rect.top <= threshold && window.scrollY > lastScrollY.current;

    if (shouldSticky && !stickyRef.current.isSticky) {
      stickyRef.current.isSticky = true;
      const placeholder = document.createElement('div');
      placeholder.style.height = `${rect.height}px`;
      if (sectionRef.current.parentNode) {
        sectionRef.current.parentNode.insertBefore(placeholder, sectionRef.current);
      }
      stickyRef.current.placeholder = placeholder;
      ...
      sectionRef.current.style.zIndex = '100';
    } else if (!shouldSticky && stickyRef.current.isSticky) {
      stickyRef.current.isSticky = false;
      sectionRef.current.style.position = '';
      sectionRef.current.style.top = '';
      sectionRef.current.style.width = '';
      if (stickyRef.current.placeholder) {
        stickyRef.current.placeholder.remove();
        stickyRef.current.placeholder = null;
      }
    }
  ...
  return (
    <section ref={sectionRef} className="snap-section py-24 bg-gradient-to-b from-slate-50 to-white relative">
