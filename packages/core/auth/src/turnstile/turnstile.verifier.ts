...
export class TurnstileVerifier {
  ...
  static fromEnv(surface: TurnstileSurface, env: NodeJS.ProcessEnv = process.env): TurnstileVerifier {
    const prefix = surface === 'admin' ? 'CF_TURNSTILE_ADMIN' : 'CF_TURNSTILE_TENANT';
  ...
  async verify(input: TurnstileVerifyInput): Promise<TurnstileSiteverifyResponse | null> {
    ...
    if (!token) {
      throw new TurnstileVerificationError('Turnstile token is required', 'missing-token');
    }
    if (token.length > MAX_TOKEN_LENGTH) {
      throw new TurnstileVerificationError('Turnstile token is too long', 'invalid-token');
    }
    if (!this.options.secretKey) {
      throw new TurnstileVerificationError('Turnstile secret key is not configured', 'missing-secret');
    }
    if (this.options.allowedHostnames.length === 0) {
      throw new TurnstileVerificationError('Turnstile allowed hostnames are not configured', 'missing-hostnames');
    }
    ...
    try {
      ...
      throw new TurnstileVerificationError(
        error instanceof Error ? error.message : 'Turnstile siteverify request failed',
        'network-error',
      );
    }

    if (!response.ok) {
      throw new TurnstileVerificationError(`Turnstile siteverify failed: ${response.status}`, 'siteverify-failed');
    }
    ...
    if (!isAllowedHostname(result.hostname, this.options.allowedHostnames)) {
      throw new TurnstileVerificationError('Turnstile hostname is not allowed', 'hostname-mismatch');
    }

    if (input.expectedAction && result.action !== input.expectedAction) {
      throw new TurnstileVerificationError('Turnstile action does not match', 'action-mismatch');
    }
...
function normalizeHostname(value: string | undefined): string {
  ...
  try {
    return new URL(/^[a-z][a-z\d+\-.]*:\/\//.test(raw) ? raw : `http://${raw}`).hostname.replace(/\.$/, '');
  } catch {
    return raw.split('/')[0]?.replace(/\.$/, '') ?? '';
  }
}

function isAllowedHostname(hostname: string | undefined, allowedHostnames: string[]): boolean {
