...
export const tenantPauseSubscriptionTool: VelaTool = {
  id:                   'tenant_pause_subscription',
  name:                 '暂停租户订阅',
  description:          '暂停指定租户的订阅，暂停后租户无法使用付费功能。操作不可自动恢复，须由运营人员手动恢复。',
  surfaces:             ['admin'],
  dataScope:            'global',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      tenantId:       { type: 'string', description: '所属租户 ID（仅用于确认摘要展示）' },
      reason:         { type: 'string', description: '暂停原因（可选，将记录至审计日志）' },
    },
...
export const tenantResumeSubscriptionTool: VelaTool = {
...
export const tenantChangePlanTool: VelaTool = {
  id:                   'tenant_change_plan',
  name:                 '调整租户套餐',
  description:          '将指定租户的订阅切换至新套餐。升级立即生效，降级在当前周期结束时生效。',
  surfaces:             ['admin'],
  dataScope:            'global',
  requiresConfirmation: true,
  confirmSummary: (input) => {
