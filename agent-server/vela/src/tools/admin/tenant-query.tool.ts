...
export const tenantSearchTool: VelaTool = {
  id:          'tenant_search',
  name:        '搜索租户',
  description: '根据租户名称或 ID 搜索平台租户，返回基本信息与订阅状态概览。关键词大小写不敏感，同时匹配名称、租户码、租户 ID。',
  surfaces:    ['admin'],
  dataScope:   'global',
  inputSchema: {
    type: 'object',
    properties: {
      query: { type: 'string', description: '搜索关键词（租户名称、租户码或租户 ID）' },
...
export const tenantDetailTool: VelaTool = {
  id:          'tenant_detail',
  name:        '租户详情',
  description: '按租户 ID 查询租户上下文详情，包含租户类型、状态、域名、工作区等信息。',
  surfaces:    ['admin'],
  dataScope:   'global',
  inputSchema: {
    type: 'object',
