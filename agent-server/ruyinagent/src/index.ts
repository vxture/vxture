    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(this.port, async () => {
        try {
          ...
          console.log(`Ruyin Agent Server running on port ${this.port}`);
          resolve();
        } catch (error) {
      ...
      if (this.server) {
        this.server.close((error) => {
          if (error) {
            ...
            console.log('Ruyin Agent Server stopped');
            resolve();
          }
        });
      } else {
...
export function startRuyinAgentServer(port?: number): Promise<RuyinAgentServer> {
  const server = new RuyinAgentServer(port);
  return server.start().then(() => server);
}

export default RuyinAgentServer;
