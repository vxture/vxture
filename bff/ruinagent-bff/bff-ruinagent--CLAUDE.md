# CLAUDE.md — @vxture/bff-ruinagent
...
| 项 | 值 |
|----|----|
| 包名 | `@vxture/bff-ruinagent` |
| 路径 | `bff/ruinagent-bff/` |
| @layer | `Application` |
| 服务对象 | `agent-studio/ruinagent` ↔ `agent-server/ruinagent` |
...
- 前端通过 HTTP 访问此 BFF
- BFF 聚合 agent-server/ruinagent（私有逻辑）和 @vxture/service-*（平台能力）
...
- 直接 import `agent-server/ruinagent` 代码（只允许 HTTP 调用）
