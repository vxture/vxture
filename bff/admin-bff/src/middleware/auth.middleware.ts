...
export class AuthMiddleware implements NestMiddleware {
  ...
  async use(req: Request, _res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (user) {
        const capabilities = await this.platformAuthService.getCapabilities(
          payload.sub,
        );
        (req as Request & RequestContext).user = user;
        (req as Request & RequestContext).capabilities = capabilities.length
          ? capabilities
          : (payload.permissions ?? []);
      }
    } catch (err) {
      this.logger.error(
        `Auth middleware error for ${req.method} ${req.path}: ${String(err)}`,
      );
