...
function makePool(connectionString: string | undefined, config: VxConfigService['database']): Pool {
  if (connectionString) {
  ...
  return new Pool({
    ...
    ssl: config.DB_SSL === 'require' ? { rejectUnauthorized: false } : undefined,
  });
}
  ...
  providers: [
    {
      ...
      useFactory: (config: VxConfigService) => makePool(undefined, config.database),
    },
    {
