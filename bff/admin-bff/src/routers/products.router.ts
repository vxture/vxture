...
type CapabilityProfileOverride = Partial<
  Omit<CapabilityProfile, "integration">
> & {
...
const capabilityProfiles: Record<string, CapabilityProfileOverride> = {
  "drone-platform": {
    description:
      "接入无人机设备、飞行任务和巡检成果，支撑低空巡检业务的设备调度与数据回传。",
    ownerTeam: "低空生态接入组",
    capabilitySummary:
      "支持无人机设备接入、任务调度、轨迹回传和巡检成果管理，是洪涝灾害监管方案的低空采集入口。",
    ...
    metrics: [
      {
        metricCode: "drone-platform.devices",
        metricName: "无人机设备数",
        unit: "台",
        cycle: "monthly",
        quotaBase: "服务套餐",
      ...
      {
        metricCode: "drone-platform.missions",
        metricName: "巡检任务数",
        unit: "次",
        cycle: "monthly",
        quotaBase: "租户订阅",
  ...
  "flood-video-interpretation": {
    ...
    ownerTeam: "模型平台组",
    capabilitySummary:
      "以视频解译模型为核心，支持多路视频分析、险情识别和结构化事件输出。",
  ...
  "report-author-agent": {
    description:
      "生成巡检报告、处置简报、法务报告和合规材料，按生成字数进行套餐配额控制。",
    ownerTeam: "智能体产品组",
    capabilitySummary:
      "提供结构化报告编制能力，可被不同业务方案复用，并按字数或报告份数计量。",
    tags: ["报告编制", "智能体", "通用能力"],
    meteringUnit: "生成字数",
  ...
  "legal-knowledge-base": {
    description:
      "管理法规、合同、案例和企业制度知识，支撑法务智能体的检索增强和权限隔离。",
    ownerTeam: "数据平台组",
  ...
  "vxture-console-cn": {
    ownerTeam: "平台产品组",
    capabilitySummary:
      "面向国内租户的运营控制台能力，承载账号、订阅、用量和基础运营工作流。",
    tags: ["Console", "平台能力"],
    meteringUnit: "席位",
  },
  ruyin: {
...
export const productAgents: ProductAgentRecord[] = [
  {
    ...
    description:
      "面向 console 平台的对话型智能助手，先用于模型策略和计量链路验证。",
    agentType: "chat",
    status: "active",
    visibility: "internal",
...
export class ProductsRouter {
  ...
  getCapability(
    ...
    const capability = listProductCapabilities().find(
...
export function listProductCapabilities(): ProductCapabilityRecord[] {
  ...
  for (const release of productReleases) {
    ...
    const seed = existing ?? {
      ...
      visibility: release.isPublic
        ? ("public" as ProductCapabilityVisibility)
        : ("internal" as ProductCapabilityVisibility),
...
function mapCapabilitySeed(seed: CapabilitySeed): ProductCapabilityRecord {
  ...
  const tierNames = new Set(
    seed.relatedSolutions.flatMap((solution) => solution.tierNames),
  );
  const releaseVersionNames = new Set(
    seed.releases.flatMap((release) => release.versionLabels),
  );
  const modelPolicyCount = listEffectiveModelPolicies().filter(
    (policy) =>
      policy.scopeType === "product" &&
      policy.productCode === seed.productCode &&
      policy.isDefined,
  ).length;
...
function inferCapabilityTypeFromRelease(
  release: ProductReleaseRecord,
): ProductCapabilityType {
  const searchText =
    `${release.productCode} ${release.productName}`.toLowerCase();
  if (searchText.includes("console") || searchText.includes("platform"))
    return "platform";
  return "agent";
}

function inferCapabilitySourceFromRelease(
  release: ProductReleaseRecord,
): ProductCapabilitySource {
  const searchText =
    `${release.productCode} ${release.productName} ${release.description}`.toLowerCase();
  return searchText.includes("partner") ||
    searchText.includes("third") ||
    searchText.includes("三方") ||
    searchText.includes("合作方")
    ? "partner"
    : "self";
}

function mergeCapabilityStatus(
  left: ProductCapabilityStatus,
  right: ProductCapabilityStatus,
): ProductCapabilityStatus {
...
function defaultCapabilitySummary(seed: CapabilitySeed): string {
  if (seed.productType === "agent")
    return "提供面向业务场景的智能体能力，可被业务产品方案组合并在套餐中配置配额。";
  if (seed.productType === "model")
    return "提供模型推理或识别能力，通过模型策略和计量规则控制调用范围。";
  if (seed.productType === "data")
    return "提供数据存储、管理或治理能力，支撑业务方案的数据沉淀和权限边界。";
  if (seed.productType === "service")
    return "提供外部服务能力接入，通常需要维护供应商、接口和服务状态。";
...
function defaultIntegration(
  ...
  const requiresIntegration =
    seed.source === "partner" ||
    seed.productType === "model" ||
    seed.productType === "service";
  ...
  return {
    ...
    status: requiresIntegration
      ? seed.status === "active"
        ...
        : "testing"
      : "not_required",
    endpoint: requiresIntegration
      ? `https://api.partner.vxture.local/${seed.productCode}`
      : null,
...
function defaultMetrics(seed: CapabilitySeed): ProductCapabilityMetricRule[] {
  if (seed.productType === "model") {
    return [
      {
        metricCode: `${seed.productCode}.requests`,
        metricName: "模型调用次数",
        unit: "次",
        cycle: "monthly",
        quotaBase: "套餐版本",
      ...
      {
        metricCode: `${seed.productCode}.channels`,
        metricName: "并发路数",
        unit: "路",
        cycle: "realtime",
        quotaBase: "服务套餐",
  ...
  if (seed.productType === "data") {
    return [
      {
        metricCode: `${seed.productCode}.storage`,
        metricName: "存储空间",
        unit: "GB",
        cycle: "monthly",
        quotaBase: "套餐版本",
      ...
      {
        metricCode: `${seed.productCode}.retention`,
        metricName: "数据保留期",
        unit: "天",
        cycle: "fixed",
        quotaBase: "服务套餐",
  ...
  if (seed.productType === "agent") {
    return [
      {
        metricCode: `${seed.productCode}.words`,
        metricName: "生成字数",
        unit: "字",
        cycle: "yearly",
        quotaBase: "服务套餐",
      ...
      {
        metricCode: `${seed.productCode}.sessions`,
        metricName: "会话次数",
        unit: "次",
        cycle: "monthly",
        quotaBase: "租户订阅",
  ...
  return [
    {
      metricCode: `${seed.productCode}.resources`,
      metricName: "资源数量",
      unit: "个",
      cycle: "monthly",
      quotaBase: "服务套餐",
    ...
    {
      metricCode: `${seed.productCode}.users`,
      metricName: "可用账号",
      unit: "人",
      cycle: "monthly",
      quotaBase: "租户订阅",
