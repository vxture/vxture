...
export class SubscriptionRouter {
  ...
  async listSubscriptions(
    ...
    const rows = await this.roPool.query<SubscriptionOperationRow>(
      SUBSCRIPTION_OPERATION_SQL,
      [CURRENT_USAGE_MONTH],
    );
  ...
  async submitSubscriptionAction(
    ...
    try {
      ...
      const currentResult = await client.query<SubscriptionActionRow>(
        SUBSCRIPTION_ACTION_LOOKUP_SQL,
        [subscriptionId],
      );
...
async function loadSubscriptionDetail(
  ...
  const [records, historyRows] = await Promise.all([
    pool.query<SubscriptionOperationRow>(SUBSCRIPTION_OPERATION_SQL, [
      CURRENT_USAGE_MONTH,
    ]),
    pool.query<SubscriptionHistoryRow>(SUBSCRIPTION_HISTORY_SQL, [
      subscriptionId,
    ]),
  ]);
...
function normalizeSubscriptionAction(
  value: unknown,
): SubscriptionOperationAction {
  if (
    value === "renew" ||
...
function subscriptionActionHistoryType(
...
function subscriptionLifecycleForAction(
  current: SubscriptionActionRow,
): SubscriptionOperationStatus {
...
function extendSubscriptionEndAt(
  ...
  const anchor =
    Number.isFinite(currentEndAt.getTime()) &&
    currentEndAt.getTime() > now.getTime()
      ? currentEndAt
      : now;
...
function inferSolution(row: SubscriptionOperationRow): {
  code: string | null;
  name: string;
} {
  const searchable =
...
function resolveSolutionAssociation(
  row: SubscriptionOperationRow,
): SubscriptionSolutionAssociation {
  ...
  if (association.solutionCode) {
    try {
      return getProductServicePlanDetail(
        association.solutionCode,
        association.tierCode,
...
function formatTokenQuota(value: number): string {
  if (value >= 100_000_000)
    return `${Math.round(value / 100_000_000)} 亿 token`;
...
function historyTone(
  status: string | null,
): SubscriptionOperationEvent["tone"] {
...
function mapSubscriptionOperationRow(
  row: SubscriptionOperationRow,
): SubscriptionOperationRecord {
  ...
  const usageRate =
    periodTokens > 0
      ? Math.min(999, Math.round((usedTokens / periodTokens) * 100))
      : usedTokens > 0
        ? 100
        : 0;
  const solution = inferSolution(row);
