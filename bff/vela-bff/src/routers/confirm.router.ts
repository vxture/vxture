...
export class ConfirmRouter {
  ...
  async confirm(
    ...
    if (!body.auditId || typeof body.auditId !== 'string') {
      throw new BadRequestException({ statusCode: 400, code: 'BAD_REQUEST', message: 'auditId is required' });
    }
    if (typeof body.confirmed !== 'boolean') {
      throw new BadRequestException({ statusCode: 400, code: 'BAD_REQUEST', message: 'confirmed must be boolean' });
    }
    ...
    try {
      upstream = await fetch(`${serverUrl}/internal/vela/confirm`, {
        ...
        body: JSON.stringify({ auditId: body.auditId, confirmed: body.confirmed, sessionId: body.sessionId }),
      });
    } catch {
      throw new HttpException(
        { statusCode: 503, code: 'SERVICE_UNAVAILABLE', message: 'Upstream service unavailable' },
